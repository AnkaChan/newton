...
- Add opt-in rigid-body penetration-free truncation to `SolverVBD` via `rigid_enable_penetration_free`: rigid pose updates and soft-body displacements are jointly truncated against per-contact division planes (Divide-and-Truncate), with curved rigid trajectories handled by sampling + bisection. Configure with `rigid_penetration_free_query_margin` (must match the collision pipeline's `soft_contact_margin`) and `rigid_conservative_bound_relaxation`.
