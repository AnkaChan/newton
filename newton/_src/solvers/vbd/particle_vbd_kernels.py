...
def solve_elasticity_tile(
    ...
    # step ratio (gamma): scales displacement update x += gamma * H^{-1} f
    step_ratio: float = 1.0,
    # per-particle squared force norm for residual tracking (pass None to skip)
    force_norm_sq_out: wp.array[float] = None,
):
    tid = wp.tid()
    ...
    if thread_idx == 0:
        ...
        if abs(wp.determinant(h_total)) > 1e-8:
            ...
            if force_norm_sq_out:
                force_norm_sq_out[particle_index] = wp.length_sq(f_total)
            particle_displacements[particle_index] = particle_displacements[particle_index] + step_ratio * h_inv * f_total
...
def solve_elasticity(
    ...
    if force_norm_sq_out:
        force_norm_sq_out[particle_index] = wp.length_sq(f)

    if abs(wp.determinant(h)) > 1e-8:
        h_inv = wp.inverse(h)
        particle_displacements[particle_index] = particle_displacements[particle_index] + step_ratio * h_inv * f
