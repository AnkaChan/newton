...
@wp.kernel
def hessian_dykstra_projection(
    # Anka's Dykstra Function, We use it here except that division planes are not precomputed and projections are hessian weighted
    max_iter: int,
    pos: wp.array(dtype=wp.vec3),
    displacement_in: wp.array(dtype=wp.vec3),
    particle_hessians: wp.array(dtype=wp.mat33),
    ...
    for iter in range(max_iter):
        ...
        for i_adj_edge in range(get_vertex_num_adjacent_edges(adjacency, particle_idx)):
            nei_edge_index, vertex_order_on_edge = get_vertex_adjacent_edge_id_order(
                adjacency, particle_idx, i_adj_edge
            )
