...
class ForceElementAdjacencyInfo:
    r"""
    - vertex_adjacent_[element]: the flatten particle_adjacency information. Its size is \sum_{i\inV} 2*N_i, where N_i is the
    ...
    - vertex_adjacent_[element]_offsets: stores where each vertex information starts in the  flatten particle_adjacency array.
    ...
    def to(self, device):
        if device == self.v_adj_faces.device:
            ...
            particle_adjacency_gpu = ForceElementAdjacencyInfo()
            particle_adjacency_gpu.v_adj_faces = self.v_adj_faces.to(device)
            particle_adjacency_gpu.v_adj_faces_offsets = self.v_adj_faces_offsets.to(device)

            particle_adjacency_gpu.v_adj_edges = self.v_adj_edges.to(device)
            particle_adjacency_gpu.v_adj_edges_offsets = self.v_adj_edges_offsets.to(device)

            particle_adjacency_gpu.v_adj_tets = self.v_adj_tets.to(device)
            particle_adjacency_gpu.v_adj_tets_offsets = self.v_adj_tets_offsets.to(device)

            particle_adjacency_gpu.v_adj_springs = self.v_adj_springs.to(device)
            particle_adjacency_gpu.v_adj_springs_offsets = self.v_adj_springs_offsets.to(device)

            return particle_adjacency_gpu


@wp.func
def get_vertex_num_adjacent_edges(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32):
    return (particle_adjacency.v_adj_edges_offsets[vertex + 1] - particle_adjacency.v_adj_edges_offsets[vertex]) >> 1


@wp.func
def get_vertex_adjacent_edge_id_order(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32, edge: wp.int32):
    offset = particle_adjacency.v_adj_edges_offsets[vertex]
    return particle_adjacency.v_adj_edges[offset + edge * 2], particle_adjacency.v_adj_edges[offset + edge * 2 + 1]


@wp.func
def get_vertex_num_adjacent_faces(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32):
    return (particle_adjacency.v_adj_faces_offsets[vertex + 1] - particle_adjacency.v_adj_faces_offsets[vertex]) >> 1


@wp.func
def get_vertex_adjacent_face_id_order(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32, face: wp.int32):
    offset = particle_adjacency.v_adj_faces_offsets[vertex]
    return particle_adjacency.v_adj_faces[offset + face * 2], particle_adjacency.v_adj_faces[offset + face * 2 + 1]


@wp.func
def get_vertex_num_adjacent_tets(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32):
    return (particle_adjacency.v_adj_tets_offsets[vertex + 1] - particle_adjacency.v_adj_tets_offsets[vertex]) >> 1


@wp.func
def get_vertex_adjacent_tet_id_order(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32, tet: wp.int32):
    offset = particle_adjacency.v_adj_tets_offsets[vertex]
    return particle_adjacency.v_adj_tets[offset + tet * 2], particle_adjacency.v_adj_tets[offset + tet * 2 + 1]


@wp.func
def get_vertex_num_adjacent_springs(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32):
    return particle_adjacency.v_adj_springs_offsets[vertex + 1] - particle_adjacency.v_adj_springs_offsets[vertex]


@wp.func
def get_vertex_adjacent_spring_id(particle_adjacency: ForceElementAdjacencyInfo, vertex: wp.int32, spring: wp.int32):
    offset = particle_adjacency.v_adj_springs_offsets[vertex]
    return particle_adjacency.v_adj_springs[offset + spring]


@wp.kernel
def _test_compute_force_element_particle_adjacency(
    particle_adjacency: ForceElementAdjacencyInfo,
    edge_indices: wp.array(dtype=wp.int32, ndim=2),
    face_indices: wp.array(dtype=wp.int32, ndim=2),
):
    wp.printf("num vertices: %d\n", particle_adjacency.v_adj_edges_offsets.shape[0] - 1)
    for vertex in range(particle_adjacency.v_adj_edges_offsets.shape[0] - 1):
        num_adj_edges = get_vertex_num_adjacent_edges(particle_adjacency, vertex)
        for i_bd in range(num_adj_edges):
            bd_id, v_order = get_vertex_adjacent_edge_id_order(particle_adjacency, vertex, i_bd)
        ...
        num_adj_faces = get_vertex_num_adjacent_faces(particle_adjacency, vertex)
...
def calculate_vertex_collision_buffer(
    ...
    for i_adj_tri in range(get_vertex_num_adjacent_faces(particle_adjacency, particle_index)):
        tri_index, _ = get_vertex_adjacent_face_id_order(particle_adjacency, particle_index, i_adj_tri)
    ...
    for i_adj_edge in range(get_vertex_num_adjacent_edges(particle_adjacency, particle_index)):
        nei_edge_index, _ = get_vertex_adjacent_edge_id_order(particle_adjacency, particle_index, i_adj_edge)
...
def compute_particle_conservative_bound(
    ...
    particle_conservative_bounds[particle_index] = particle_conservative_bound_relaxation * min_dist
...
def solve_trimesh_no_self_contact(
    ...
    if tri_indices:
        # elastic force and hessian
        for i_adj_tri in range(get_vertex_num_adjacent_faces(particle_adjacency, particle_index)):
            tri_id, particle_order = get_vertex_adjacent_face_id_order(particle_adjacency, particle_index, i_adj_tri)
    ...
    if tet_indices:
        for i_adj_tet in range(get_vertex_num_adjacent_tets(particle_adjacency, particle_index)):
            nei_tet_index, vertex_order_on_tet = get_vertex_adjacent_tet_id_order(particle_adjacency, particle_index, i_adj_tet)
...
def hessian_dykstra_projection( # Anka's Dykstra Function, We use it here except that division planes are not precomputed and projections are hessian weighted
    ...
    for iter in range(max_iter):
        ...
        for i_adj_tri in range(get_vertex_num_adjacent_faces(particle_adjacency, particle_idx)):
            tri_index, vertex_order = get_vertex_adjacent_face_id_order(particle_adjacency, particle_idx, i_adj_tri)
            t1 = pos[tri_indices[tri_index, 0]]
            t2 = pos[tri_indices[tri_index, 1]]
            t3 = pos[tri_indices[tri_index, 2]]
        ...
        for i_adj_edge in range(get_vertex_num_adjacent_edges(particle_adjacency, particle_idx)):
            nei_edge_index, vertex_order_on_edge = get_vertex_adjacent_edge_id_order(particle_adjacency, particle_idx,
                                                                                    i_adj_edge)
...
def _csr_row(vals: np.ndarray, offs: np.ndarray, i: int) -> np.ndarray:
    """Extract CSR row `i` from the flattened particle_adjacency arrays."""
    return vals[offs[i] : offs[i + 1]]
...
def one_ring_vertices(
    ...
    Args:
        ...
        v_adj_edges: Flattened CSR particle_adjacency array listing edge ids and local order.
...
def leq_n_ring_vertices(
    ...
    Args:
        ...
        v_adj_edges: CSR values for vertex-edge particle_adjacency
        v_adj_edges_offsets: CSR offsets for vertex-edge particle_adjacency
...
def accumulate_spring_force_and_hessian(
    ...
    num_adj_springs = get_vertex_num_adjacent_springs(particle_adjacency, particle_index)
    for spring_counter in range(num_adj_springs):
        spring_index = get_vertex_adjacent_spring_id(particle_adjacency, particle_index, spring_counter)
...
class SolverVBD(SolverBase):
    ...
    def __init__(
        ...
        particle_particle_q_rest_contact_exclusion_radius: float = 0.0,
        external_vertex_contact_filtering_map: dict | None = None,
        external_edge_contact_filtering_map: dict | None = None,
        integrate_with_external_rigid_solver: bool = False,
        penetration_free_particle_conservative_bound_relaxation: float = 0.42,
        ...
        use_particle_tile_solve: bool = True,
        ...
        Args:
            ...
            particle_enable_self_contact: whether to self-contact.
            particle_self_contact_radius: The radius used for self-contact detection. This is the distance at which vertex-triangle
                pairs and edge-edge pairs will start to interact with each other.
            particle_self_contact_margin: The margin used for self-contact detection. This is the distance at which vertex-triangle
                pairs and edge-edge will be considered in contact generation. It should be larger than `particle_self_contact_radius`
            ...
            particle_particle_q_rest_contact_exclusion_radius: Additional world-space distance threshold for filtering topologically close
            ...
            penetration_free_particle_conservative_bound_relaxation: Relaxation factor for conservative penetration-free projection.
            ...
            particle_collision_detection_interval: Controls how frequently collision detection is applied during the simulation.
            ...
            use_particle_tile_solve: whether to accelerate the solver using tile API
        Note:
        ...
        self.particle_particle_q_rest_contact_exclusion_radius = particle_particle_q_rest_contact_exclusion_radius
        ...
        self.particle_adjacency = self.compute_force_element_particle_adjacency(model).to(self.device)
        ...
        if model.device.is_cpu and use_particle_tile_solve:
            warnings.warn("Tiled solve requires model.device='cuda'. Tiled solve is disabled.", stacklevel=2)

        self.use_particle_tile_solve = use_particle_tile_solve and model.device.is_cuda
        ...
        if particle_enable_self_contact:
            ...
            self.particle_conservative_bound_relaxation = penetration_free_particle_conservative_bound_relaxation
        ...
        # tests
        # wp.launch(kernel=_test_compute_force_element_particle_adjacency,
        #           inputs=[self.particle_adjacency, model.edge_indices, model.tri_indices],
    ...
    def compute_force_element_particle_adjacency(self, model):
        particle_adjacency = ForceElementAdjacencyInfo()
    ...
    def simulate_one_step_no_tile(
        ...
        for _iter in range(self.iterations):
            ...
            if (self.particle_collision_detection_interval == 0 and _iter == 0) or (
                self.particle_collision_detection_interval >= 1 and _iter % self.particle_collision_detection_interval == 0
            ):
                self.collision_detection_penetration_free(state_in)
    ...
    def collision_detection_penetration_free(self, current_state: State):
        ...
        if self.particle_enable_self_contact:
            ...
            self.trimesh_collision_detector.vertex_triangle_collision_detection(
                ...
                min_query_radius=self.particle_particle_q_rest_contact_exclusion_radius,
    ...
    def penetration_free_truncation(self, particle_q_out=None):
        ...
        if not self.particle_enable_self_contact:
            ...
            if self.truncation_mode == 0:
                ...
                #         self.particle_self_contact_margin * self.particle_conservative_bound_relaxation,
                #     ],
                #     outputs=[
                #         self.particle_displacements,  # particle_displacements: wp.array(dtype=wp.vec3),
                #         particle_q_out,
    ...
    def penetration_free_truncation_ccd(self, particle_q_out=None):
        ...
        if self.ccd_detector is None:
            # CCD builds its own filter lists using the particle_adjacency info
            # This is independent of DCD's particle_topological_contact_filter_threshold
            self.ccd_detector = TriMeshContinuousCollisionDetector(
                self.trimesh_collision_detector,
                self.pos_prev_collision_detection,
                self.particle_displacements,
                particle_adjacency=self.particle_adjacency,  # Pass particle_adjacency for building CCD's own filter lists
