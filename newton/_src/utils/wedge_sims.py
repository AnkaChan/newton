...
OUTPUT_FOLDER = "D:\Data\GTC2025DC_Demo\B1021"
start = 3
...
def run_cloth_sim(config, output_folder):
    ...
    config["output_usd"] =  os.path.join(output_folder, Path(config["input_usd"]).stem + "_" + config["wedge_number"] + ".usd")
...
def init_configs():
    ...
    for idx, param_combo in enumerate(all_param_combos):
        ...
        settings_name = "_".join(
            [f"{k}={v}" for k, v in zip(abbr_param_keys, param_combo)]
        )
  
        new_config["do_process"] = True
        new_config["text"] = settings_name
        new_config["wedge_number"] = f"sweep_" + str(idx).zfill(3)
...
def render_usds(test_cases):
    for config in test_cases.values():
        ...
        render_usd = [
            ...
            "-o", config['clips_basepath'],  # output path
            "-n", str(config['n']),  # num_frames
            "-t", str(config['t']),  # sim time
            "-f", str(config['frame_rate']),  # frame rate
        ]
        subprocess.run(render_usd)
...
def clip_path_to_mp4_impl(clip_name, clip_dir, config, rgb_path, image_base_name, output_path):
    if not os.path.exists(output_path):
        cmd = [
            'ffmpeg',
            '-framerate', '60',
            '-i', f'{rgb_path}/{image_base_name}%04d.png',
            '-c:v', 'libx264',
            '-pix_fmt', 'yuv420p',
            output_path
        ]
...
def plain_clips_to_annotated_clips(test_cases):
    for config in test_cases.values():
        ...
        for clip_config in config["clips_plain"]:
            ...
            clip_to_annotated_mp4_impl(clip_config["full_path"], config["text"], clip_config["dir"], annotated_name, output_path, config, "center")
...
def plain_clips_to_2x2_grid_impl(name, config):
    ...
    config["clips_2x2"] = [{
        "name": name_clip_2x2,
        "dir": config["clips_basepath"],
        "full_path": full_path_clip_2x2
    }]
...
def annotate_2x2_grid(test_cases):
    for config in test_cases.values():
        ...
        for clip_config in config["clips_2x2"]:
            annotated_name = f'{clip_config["name"]}_annotated.mp4'
            output_path = f'{clip_config["dir"]}/{annotated_name}'
            clip_to_annotated_mp4_impl(input=clip_config["full_path"], text=config["text"], dir=clip_config["dir"], annotated_name=annotated_name, output_path=output_path, config=config, align="left")
