...
def configure_capture(
    ...
    *,
    ...
    if owner.capture_replay and owner.capture_frames > 0:
        ...
        _log(
            owner,
            f"  PNG frames will be written here; replay.{owner.capture_format} is stitched at the end.",
        )
...
def capture_replay_frame(
    ...
    *,
    ...
    if owner.capture_count % 20 == 0:
        print(f"{_LOG_PREFIX} saved {owner.capture_count}/{target_count} frames")
...
def should_hold_simulation_for_capture(owner: Any) -> bool:
    ...
    if not getattr(owner, "capture_replay", False) or getattr(owner, "capture_done", False):
        return False
...
def finalize_replay_video(owner: Any) -> None:
    ...
    try:
        ...
        print(f"{_LOG_PREFIX} mp4 failed ({exc}); wrote gif: {fallback}")
...
def trim_replay_capture(
    ...
    *,
    ...
    if owner.capture_dir is None:
        ...
        owner._last_captured_frame_key = owner.capture_count - 1 if owner.capture_count > 0 else None
        return
    ...
    owner.capture_done = False
    owner.capture_video_path = None
    owner._last_captured_frame_key = keep_count - 1 if keep_count > 0 else None
    target_count = _target_frame_count(owner, target_frame_count)
    print(f"{_LOG_PREFIX} trimmed to {keep_count}/{target_count} accepted frames")
