...
# fits within one frame with margin, and gives ~3x more sag than the hard bag.
_SOFT_TRI_KE = 3.0e4
_SOFT_TRI_KA = 3.0e4
_SOFT_TRI_KD = 1.7e-5
_SOFT_EDGE_KE = 5.0e2
...
@wp.kernel
def _k_bary_interp(
    phys_q: wp.array[wp.vec3],
    vi0: wp.array[wp.int32],
    vi1: wp.array[wp.int32],
    vi2: wp.array[wp.int32],
    bary: wp.array[wp.vec3],
    disp: wp.array[wp.vec3],
    scale: float,
...
def _fit_bag_contents(phys_verts_cm):
    ...
    Placement strategy (verified against physics proxy mesh):
      - Sphere (R=4) and box (6x6x6) sit side-by-side in X at the same height.
    ...
    def dist_sphere(ox, oy, oz, r):
        d = np.sqrt((phys_verts_cm[:, 0] - ox) ** 2 + (phys_verts_cm[:, 1] - oy) ** 2 + (phys_verts_cm[:, 2] - oz) ** 2)
    ...
    def dist_cap_y(ox, oy, oz, r, hl):
        ...
        d = np.sqrt((phys_verts_cm[:, 0] - ox) ** 2 + (phys_verts_cm[:, 1] - vy) ** 2 + (phys_verts_cm[:, 2] - oz) ** 2)
        return float(d.min()) - r
...
class Example:
    ...
    def __init__(
        ...
        self._bary_vi0_np, self._bary_vi1_np, self._bary_vi2_np, bary_w = _build_bary_map(
            full_verts_cm, phys_verts_cm, phys_faces
        )
        ...
        self._bary_disp = wp.array(
            (full_verts_cm - bary_proj).astype(np.float32),
            dtype=wp.vec3,
        )
        ...
        builder.add_body(
            xform=wp.transform(wp.vec3(s_x, s_y, s_z + _DROP_HEIGHT), wp.quat_identity()),
            mass=0.1,
        )
        builder.add_shape_sphere(body=0, radius=4.0, cfg=content_cfg)

        builder.add_body(
            xform=wp.transform(wp.vec3(b_x, b_y, b_z + _DROP_HEIGHT), wp.quat_identity()),
            mass=0.1,
        )
        builder.add_shape_box(body=1, hx=3.0, hy=3.0, hz=3.0, cfg=content_cfg)

        builder.add_body(
            xform=wp.transform(wp.vec3(c_x, c_y, c_z + _DROP_HEIGHT), cap_quat),
            mass=0.1,
        )
        ...
        bag_verts_wp = [wp.vec3(float(v[0]), float(v[1]), float(v[2])) for v in phys_verts_cm]
        ...
        self._proxy_indices_wp = wp.array(
            (phys_faces.flatten() + ps).astype(np.int32),
            dtype=wp.int32,
        )
        ...
        if self.soft_bag:
            ...
            _pgrnd_cfg = newton.ModelBuilder.ShapeConfig(
                ke=_SOFT_PARTICLE_GROUND_KE, kd=_SOFT_PARTICLE_GROUND_KD, mu=0.4, has_shape_collision=False
            )
            ...
            _rgrnd_cfg = newton.ModelBuilder.ShapeConfig(
                ke=_SOFT_RIGID_GROUND_KE, kd=_SOFT_RIGID_GROUND_KD, mu=0.4, has_particle_collision=False
            )
        ...
        bag_mass = float(all_mass[self._bag_particle_start : self._bag_particle_end].sum())
        object_masses = 1.0 / inv_m[:3]

        print(
            f"{_LOG_PREFIX} Bodies: {self.model.body_count}, "
        ...
        self._handle_inv_mass_orig_wp = wp.array(all_inv_m[self._handle_global].astype(np.float32), dtype=float)
        self._handle_mass_orig_wp = wp.array(all_mass[self._handle_global].astype(np.float32), dtype=float)

        # Pin handles now (will release after _HANG_FRAMES)
        self._is_hanging = True
        self._frame_count = 0
        n_h = len(self._handle_global)
        wp.launch(_k_set_float_scalar, dim=n_h, inputs=[self.model.particle_inv_mass, self._handle_idx_wp, 0.0])
        wp.launch(_k_set_float_scalar, dim=n_h, inputs=[self.model.particle_mass, self._handle_idx_wp, 0.0])

        print(f"{_LOG_PREFIX} Handle particles: {n_h} (z >= {z_top_thresh:.1f} cm), hanging for {_HANG_FRAMES} frames")
    ...
    def step(self):
        ...
        if self._is_hanging and self._frame_count > _HANG_FRAMES:
            ...
            wp.launch(
                _k_set_float_array,
                dim=n_h,
                inputs=[self.model.particle_mass, self._handle_idx_wp, self._handle_mass_orig_wp],
            )
            wp.launch(
                _k_set_float_array,
                dim=n_h,
                inputs=[self.model.particle_inv_mass, self._handle_idx_wp, self._handle_inv_mass_orig_wp],
            )
    ...
    def test_final(self):
        ...
        assert float(bag_z.max()) < _BAG_H + 5.0, f"Bag top too high: {float(bag_z.max()):.1f} cm"
        assert float(bag_z.min()) >= -1.0, f"Bag penetrated ground: {float(bag_z.min()):.1f} cm"
