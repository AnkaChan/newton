...
def require_trimesh():
    ...
    try:
        ...
        raise ImportError(
            "This example requires trimesh. Install the examples/importers extras "
            "or run `uv pip install trimesh`."
        ) from exc
    return trimesh
...
def require_imageio():
    ...
    try:
        ...
        raise ImportError(
            "Replay capture requires imageio. Install with "
...
def _fit_bag_contents(phys_verts_cm: np.ndarray):
    ...
    def dist_sphere(cx: float, cy: float, cz: float, radius: float) -> float:
        dist = np.sqrt(
            (phys_verts_cm[:, 0] - cx) ** 2
            + (phys_verts_cm[:, 1] - cy) ** 2
            + (phys_verts_cm[:, 2] - cz) ** 2
        )
...
def _kw_fixed_field(value: int | float | None, width: int) -> str:
    ...
    if value is None:
        ...
        for candidate in candidates:
            if "e" not in candidate and "E" not in candidate and "." not in candidate:
                candidate += ".0"
...
def _relative_euler_xyz_rad(quats_xyzw: np.ndarray) -> np.ndarray:
    ...
    relative_quats = np.stack(
        [
            (Rotation.from_quat(quat_xyzw) * initial_inv).as_quat()
            for quat_xyzw in quats_xyzw
        ],
        axis=0,
    )
...
def _attached_bag_node_local_indexes(
    ...
    left_center_m, left_half_extents_m = _axis_aligned_box_from_vertices(
        left_pad_vertices_m
    )
    right_center_m, right_half_extents_m = _axis_aligned_box_from_vertices(
        right_pad_vertices_m
    )
    ...
    for margin_scale in _GRIP_ATTACH_MARGIN_SCALES:
        ...
        signed_offsets = (
            bag_vertices_m[candidate_union] - grip_midpoint_m[None, :]
        ) @ grip_axis_m
        ...
        if len(left_indexes) >= _GRIP_ATTACH_TARGET_NODE_COUNT and len(
            right_indexes
    ...
    if len(left_indexes) == 0 or len(right_indexes) == 0:
        raise RuntimeError(
            "Could not identify bag vertices inside both finger pad boxes for the "
            "rigid attachment grip."
        )
...
def _lsdyna_debug_hint(job_dir: Path) -> str:
    ...
    return (
        ...
        f"`{job_dir / 'messag'}`, and `{_lsdyna_debug_summary_path(job_dir)}` for details."
    )
...
def _build_lsdyna_debug_summary(
    *,
    ...
    return {
        ...
        "last_d3plot_write_time_s": (
            None if log_d3plot_times_s.size == 0 else float(log_d3plot_times_s[-1])
        ),
        ...
        "stream_solver_exit_code": (
            None if stream_solver_exit_code is None else int(stream_solver_exit_code)
        ),
...
def _write_keyword_deck(
    ...
    for part in parts:
        ...
        _append_keyword_block(
            ...
            [_kw_fixed_line([part.part_id, 1, 2, curve_id, 1.0, 0, active_until_s, 0.0], [10, 10, 10, 10, 10, 10, 10, 10])],
        )
        ...
        _append_keyword_block(
            deck,
            "*BOUNDARY_PRESCRIBED_MOTION_RIGID",
            [_kw_fixed_line([part.part_id, 2, 2, curve_id, 1.0, 0, active_until_s, 0.0], [10, 10, 10, 10, 10, 10, 10, 10])],
        )
        ...
        _append_keyword_block(
            deck,
            "*BOUNDARY_PRESCRIBED_MOTION_RIGID",
            [_kw_fixed_line([part.part_id, 3, 2, curve_id, 1.0, 0, active_until_s, 0.0], [10, 10, 10, 10, 10, 10, 10, 10])],
        )
        ...
        if part.prescribed_rotation_rad is not None:
            for axis, dof, label in ((0, 5, "rx"), (1, 6, "ry"), (2, 7, "rz")):
                ...
                _append_define_curve(
                    ...
                    curve_id,
                    np.column_stack(
                        [motion.times_s, part.prescribed_rotation_rad[:, axis]]
                    ),
            ...
            for dof in (5, 6, 7):
                _append_keyword_block(
                    deck,
                    "*BOUNDARY_PRESCRIBED_MOTION_RIGID",
                    [_kw_fixed_line([part.part_id, dof, 2, zero_curve_id, 1.0, 0, active_until_s, 0.0], [10, 10, 10, 10, 10, 10, 10, 10])],
                )
    ...
    for part in parts:
        _append_keyword_block(
            ...
            [
                part.label,
                _kw_fixed_line([part.part_id, part.section_id, part.material_id, 0, 0, 0, 0, 0], [10, 10, 10, 10, 10, 10, 10, 10]),
            ],
        )
        _append_keyword_block(
            deck,
            ...
            [
                ...
                _kw_fixed_line([part.thickness_m, part.thickness_m, part.thickness_m, part.thickness_m, 0.0, 0.0, 0.0, 0], [10, 10, 10, 10, 10, 10, 10, 10]),
            ],
        )
        if part.rigid:
            _append_keyword_block(
                ...
                "*MAT_RIGID",
                [
                    _kw_fixed_line([part.material_id, part.density_kg_m3, part.youngs_modulus_pa, part.poisson_ratio, 0.0, 0.0, 0.0, None], [10, 10, 10, 10, 10, 10, 10, 10]),
            ...
            _append_keyword_block(
                ...
                "*MAT_ELASTIC",
                [
                    _kw_fixed_line([part.material_id, part.density_kg_m3, part.youngs_modulus_pa, part.poisson_ratio, None, None, None], [10, 10, 10, 10, 10, 10, 10]),
                ],
            )
    ...
    print(
        f"{_LOG_PREFIX} Attached grip nodes: "
        f"left={len(left_attach_node_ids)} right={len(right_attach_node_ids)}"
    )
    _append_keyword_block(
        deck,
...
def _extract_shell_part_topology(
    ...
    global_to_local = {
        ...
        for local_index, global_index in enumerate(global_node_indexes.tolist())
    }
    faces = np.array(
        [
...
def _lsdyna_log_has_normal_termination(log_path: Path) -> bool:
    ...
    try:
        ...
        return False
    return (
        "N o r m a l    t e r m i n a t i o n" in text
        or "*** termination time reached ***" in text
    )
...
def _resolve_replay_times_s(
    ...
    if raw_times_s.size == state_count and len(_valid_state_indexes_from_times(raw_times_s, output_dt_s)) == state_count:
...
def _load_replay_data_with_diagnostics(
    ...
    if ArrayType.node_displacement not in arrays:
        raise RuntimeError(
            f"The d3plot file does not contain nodal displacements. {_lsdyna_debug_hint(job_dir)}"
        )
    if ArrayType.element_shell_stress not in arrays:
        raise RuntimeError(
            f"The d3plot file does not contain shell stress output. {_lsdyna_debug_hint(job_dir)}"
        )
    ...
    if ArrayType.rigid_body_coordinates in arrays and ArrayType.rigid_body_rotation_matrix in arrays:
        ...
        if geometry_state_indexes is not None and len(geometry_state_indexes) > 0 and len(geometry_state_indexes) < rigid_coords.shape[0]:
...
def _initial_replay_data(shell_verts_cm: np.ndarray, deck: DeckMetadata) -> ReplayData:
    ...
    initial_body_q_cm = {
        label: pose[None, :].astype(np.float32)
        for label, pose in deck.content_part_initial_q_cm.items()
    }
    return ReplayData(
...
def _resample_replay_data(
    ...
    *,
    ...
    aligned = ReplayData(
        ...
        rigid_body_q_cm={
            label: values[source_indexes].astype(np.float32)
...
class Example:
    ...
    def __init__(self, viewer, args):
        ...
        self.capture_frames = (
            ...
            if requested_capture_frames == _DEFAULT_NUM_FRAMES
    ...
    def _update_replay_from_source(self, *, hold_last: bool):
        ...
        if used_tail_hold and not self._tail_hold_warned and len(self._source_replay.times_s) > 0:
            ...
            target_end_s = float(self._target_frame_times_s[-1]) if len(self._target_frame_times_s) > 0 else source_end_s
            print(
    ...
    def _poll_streaming_replay(self, *, force: bool = False):
        ...
        try:
            ...
            if self._stream_solver_finished:
                ...
                raise RuntimeError(
                    f"LS-DYNA completed without producing a d3plot file. "
                    f"{_lsdyna_debug_hint(self.job_dir)}"
                )
            return
        ...
        if self._stream_solver_finished:
            ...
            if self._stream_solver_exit_code not in (0, None):
                ...
                if _lsdyna_log_has_normal_termination(log_path):
                    ...
                    raise RuntimeError(
                        f"LS-DYNA exited with code {self._stream_solver_exit_code}. "
                        f"{_lsdyna_debug_hint(self.job_dir)}"
                    )
    ...
    def step(self):
        ...
        if self._stream_mode and self._stream_complete and not self.capture_replay and hasattr(self.viewer, "num_frames"):
            self._stop_viewer()
    ...
    def render(self):
        ...
        visual_bag_points_wp = wp.array(self._frame_visual_bag_points_m(frame_index), dtype=wp.vec3, device=self.model.device)
    ...
    def test_final(self):
        ...
        assert initial_top_z_cm > 30.0, (
            "Lift-start replay did not start lifted: "
            f"top z = {initial_top_z_cm:.1f} cm "
            "(expected > 30 cm)"
        )
        assert final_top_z_cm > 20.0, (
            "Lift-start replay dropped too far during replay: "
            f"top z = {final_top_z_cm:.1f} cm "
            "(expected > 20 cm)"
        )
...
def _finalize_rollback_summary_ranges(summary: dict[str, object]) -> None:
    ...
    for event_index, event in enumerate(events):
        ...
        event["final_retained_frame_start_index"] = range_record[
            "start_frame_index"
        ]
        event["final_retained_frame_start_number"] = range_record[
            "start_frame_number"
        ]
        event["final_retained_frame_end_index"] = range_record["end_frame_index"]
        event["final_retained_frame_end_number"] = range_record[
            "end_frame_number"
        ]
...
def _initial_lift_content_body_q_cm(
    ...
    sphere_pos_cm, box_pos_cm, capsule_pos_cm, capsule_quat_wp = (
        _fit_lift_bag_contents(bag_verts_cm)
    )
    return {
        "sphere": np.array(
...
class LiftRobotTrajectorySampler(_ansys_common.RobotTrajectorySampler):
    ...
    def _set_joint_targets(self):
        ...
        current = self.waypoints[self._current_waypoint]
        nxt = self.waypoints[
            min(self._current_waypoint + 1, len(self.waypoints) - 1)
        ]
        alpha = min(self._time_in_waypoint / current[1], 1.0)
        phase_alpha = (
            self._smoothstep(alpha)
            if self._current_waypoint == self._LIFT_WAYPOINT_INDEX
            else alpha
        )

        target_pos = current[0] * (1.0 - phase_alpha) + nxt[0] * phase_alpha
        self._gripper_frac = (
            float(current[2]) * (1.0 - phase_alpha)
            + float(nxt[2]) * phase_alpha
        )

        rot = wp.quat_from_axis_angle(wp.vec3(1.0, 0.0, 0.0), float(np.pi))
        self._pos_obj.set_target_positions(
            wp.array([target_pos], dtype=wp.vec3)
        )
        self._rot_obj.set_target_rotations(
            wp.array([_ansys_common._qv4(rot)], dtype=wp.vec4)
        )
        self._ik_solver.step(self._joint_q_ik, self._joint_q_ik, iterations=24)

        if (
            self._time_in_waypoint >= current[1]
    ...
    *,
    ...
    for part in parts:
        ...
        for axis, dof, label in ((0, 1, "dx"), (1, 2, "dy"), (2, 3, "dz")):
            ...
            _ansys_common._append_define_curve(
                ...
                curve_id,
                np.column_stack(
                    [motion.times_s, part.prescribed_displacement_m[:, axis]]
                ),
                title=f"{part.label}_{label}",
            )
            _ansys_common._append_keyword_block(
    ...
    return _ansys_common.DeckMetadata(
        ...
        content_part_initial_q_cm={
            label: np.asarray(pose, dtype=np.float32).copy()
            for label, pose in content_part_initial_q_cm.items()
        },
    )
...
def _slice_motion_samples(
    ...
    start_frame_index = int(
        np.clip(start_frame_index, 0, len(motion.times_s) - 1)
    )
    if end_frame_exclusive is None:
        end_frame_exclusive = len(motion.times_s)
    end_frame_exclusive = int(
        np.clip(end_frame_exclusive, start_frame_index + 1, len(motion.times_s))
    )
    start_time_s = float(motion.times_s[start_frame_index])
    return _ansys_common.MotionSamples(
        times_s=(
            motion.times_s[start_frame_index:end_frame_exclusive] - start_time_s
        ).astype(np.float32),
        robot_body_q_cm=motion.robot_body_q_cm[
            start_frame_index:end_frame_exclusive
        ]
        .astype(np.float32)
        .copy(),
        left_pad_centers_m=motion.left_pad_centers_m[
            start_frame_index:end_frame_exclusive
        ]
        .astype(np.float32)
        .copy(),
        right_pad_centers_m=motion.right_pad_centers_m[
            start_frame_index:end_frame_exclusive
        ]
        .astype(np.float32)
        .copy(),
        left_pad_quat_xyzw=motion.left_pad_quat_xyzw[
            start_frame_index:end_frame_exclusive
        ]
        .astype(np.float32)
        .copy(),
        right_pad_quat_xyzw=motion.right_pad_quat_xyzw[
            start_frame_index:end_frame_exclusive
        ]
        .astype(np.float32)
        .copy(),
...
def _truncate_replay(
    ...
    return _ansys_common.ReplayData(
        ...
        bag_points_m=replay.bag_points_m[:frame_count]
        .astype(np.float32)
        .copy(),
        bag_von_mises=replay.bag_von_mises[:frame_count]
        .astype(np.float32)
        .copy(),
        rigid_body_q_cm={
            label: values[:frame_count].astype(np.float32).copy()
            for label, values in replay.rigid_body_q_cm.items()
        },
    )
...
class Example(_ansys_common.Example):
    ...
    def create_parser():
        ...
        parser.add_argument(
            ...
            default=True,
            help=(
                "Stop after the first failed LS-DYNA attempt instead of "
                "retrying from accepted replay frames."
            ),
        )
        parser.add_argument(
        ...
        parser.add_argument(
            ...
            help=(
                "Multiplier applied to TSSFAC on each retry, e.g. 0.75 turns "
                "0.90 into 0.675, then 0.506, and so on."
            ),
        )
        parser.add_argument(
            "--rollback-min-tssfac",
    ...
    def __init__(self, viewer, args):
        closed_width_cm = float(getattr(args, "closed_width_cm"))
        rollback_enabled = bool(getattr(args, "rollback", True))
        rollback_max_retries = int(getattr(args, "rollback_max_retries"))
        if not rollback_enabled:
            rollback_max_retries = 0
        rollback_backtrack_frames = int(
            getattr(args, "rollback_backtrack_frames")
        )
        rollback_base_tssfac = float(getattr(args, "rollback_base_tssfac"))
        rollback_tssfac_scale = float(getattr(args, "rollback_tssfac_scale"))
        rollback_min_tssfac = float(getattr(args, "rollback_min_tssfac"))

        if rollback_max_retries < 0:
            raise ValueError("`--rollback-max-retries` must be non-negative.")
        if rollback_backtrack_frames < 0:
            raise ValueError(
                "`--rollback-backtrack-frames` must be non-negative."
            )
        if (
            not np.isfinite(rollback_base_tssfac)
            or rollback_base_tssfac <= 0.0
        ):
            raise ValueError("`--rollback-base-tssfac` must be positive.")
        if (
            not np.isfinite(rollback_tssfac_scale)
            or rollback_tssfac_scale <= 0.0
        ):
        ...
        print(
            f"{_ansys_common._LOG_PREFIX} Friction grip final finger gap: "
            f"{closed_width_cm:.2f} cm"
        )
        print(
        ...
        self.job_dir = Path(args.job_dir)
        self.job_dir.mkdir(parents=True, exist_ok=True)
        self._lsdyna_debug_summary_path = _ansys_common._lsdyna_debug_summary_path(
            self.job_dir
        )
        ...
        print(
            f"{_ansys_common._LOG_PREFIX} Rollback summary: "
        ...
        if (
            ...
            or self.capture_frames != self._full_capture_frames
        ):
            print(
        ...
        self._target_frame_times_s = (
            self.motion.times_s[: self.capture_frames]
            .astype(np.float32)
            .copy()
        )
        ...
        self.model, self._content_body_indices, self._robot_body_count = (
            _ansys_common._build_visual_model(
                shell_verts_cm,
                small_pad=self.small_pad,
            )
        )
        self._initial_body_q_cm = self.model.body_q.numpy().copy().astype(
            np.float32
        )
        self.viz_state = self.model.state()
        self.state_0 = self.viz_state
    ...
    def _persist_rollback_summary(self) -> None:
        ...
        self._rollback_summary["final_replay_frame_count"] = int(
            replay_frame_count
        )
    ...
    def _register_rollback_attempt(
        ...
        *,
        ...
        accepted_frame_count = (
            len(self.replay.times_s) if hasattr(self, "replay") else 0
        )
        ...
        self._rollback_summary["attempt_count"] = len(
            self._rollback_summary["attempts"]
        )
        ...
        if attempt_mode == "rescue":
            retry_event: dict[str, object] = {
                "retry_index": int(
                    len(self._rollback_summary["rollback_events"]) + 1
                ),
                "attempt_index": int(attempt_index),
                "attempt_dir": attempt_dir.name,
                "had_committed_prefix": had_committed_prefix,
            ...
            self._rollback_summary["retry_event_count"] = len(
                self._rollback_summary["rollback_events"]
            )
    ...
    def _close_stream_solver_log_file(self) -> None:
        ...
        if (
            self._stream_solver_log_file is not None
            and not self._stream_solver_log_file.closed
        ):
            self._stream_solver_log_file.close()
        self._stream_solver_log_file = None
        ...
        *,
        ...
        if attempt_mode == "rescue":
            tssfac = max(
                ...
                float(self._rollback_base_tssfac)
                * (float(self._rollback_tssfac_scale) ** int(rescue_depth)),
            )
        ...
        self._rollback_current_time_offset_s = float(
        ...
        if attempt_index == 0 and attempt_mode == "base":
            if self._rollback_enabled and self._rollback_max_retries > 0:
                ...
                print(
                    f"{_ansys_common._LOG_PREFIX} LS-DYNA single attempt "
                    f"TSSFAC={tssfac:.3f} (rollback disabled)"
                )
    ...
    def _record_current_attempt_outcome(
        ...
        *,
        ...
        restart_frame_index = int(
            self._rollback_active_attempt_record["restart_frame_index"]
        )
        ...
        self._rollback_active_attempt_record["normal_termination"] = bool(
            normal_termination
        )
        self._rollback_active_attempt_record["exit_observation"] = str(
            exit_observation
        )
        self._rollback_active_attempt_record["aligned_frame_count"] = int(
            appended_frame_count
        )
        self._rollback_active_attempt_record["final_append_count"] = int(
            final_append_count
        )
        self._rollback_active_attempt_record["merged_frame_count_after_attempt"] = int(
            accepted_frame_count
        )
        ...
        if self._rollback_active_retry_event is not None:
            ...
            self._rollback_active_retry_event["normal_termination"] = bool(
                normal_termination
            )
            self._rollback_active_retry_event["exit_observation"] = str(
                exit_observation
            )
            self._rollback_active_retry_event["aligned_frame_count"] = int(
                appended_frame_count
            )
            self._rollback_active_retry_event["final_append_count"] = int(
                final_append_count
            )
            self._rollback_active_retry_event["merged_frame_count_after_attempt"] = int(
                accepted_frame_count
            )
    ...
    def _append_final_attempt_replay_update(self, attempt_dir: Path) -> int:
        ...
        try:
            update, self._last_replay_diagnostics = (
            ...
            raise RuntimeError(
                "Failed to read final LS-DYNA d3plot output after the solver "
                f"finished. {exc}"
            ) from exc
        ...
        if appended > 0:
            ...
            print(
                f"{_ansys_common._LOG_PREFIX} Received "
                f"{len(self.replay.times_s)}/{total_frames} replay frames"
            )
            self._persist_rollback_summary()
        return int(appended)
    ...
    def _prepare_retry_from_prefix(
        ...
        if self._rollback_active_attempt_record is not None:
            self._rollback_active_attempt_record[
                "scheduled_next_restart_frame_index"
            ] = int(restart_frame_index)
            self._rollback_active_attempt_record[
                "scheduled_next_restart_frame_number"
            ] = int(restart_frame_index + 1)
            self._rollback_active_attempt_record[
                "scheduled_next_restart_time_s"
            ] = float(self.motion.times_s[restart_frame_index])
        ...
        restart_bag_verts_cm = (
            self.replay.bag_points_m[restart_frame_index] * 100.0
        ).astype(np.float32)
    ...
    def _finish_or_retry_current_attempt(self) -> None:
        ...
        log_path = self._stream_solver_log_path or (
            current_attempt_dir / "lsdyna.stdout.txt"
        )
        self._close_stream_solver_log_file()

        normal_termination = _ansys_common._lsdyna_log_has_normal_termination(log_path)
        exit_code = 0 if self._stream_solver_exit_code in (None, 0) else int(
            self._stream_solver_exit_code
        )
        final_append_count = self._append_final_attempt_replay_update(
            current_attempt_dir
        )
        ...
        if self._rollback_current_attempt_mode == "rescue" and produced_new_frames:
            ...
            if self._rollback_active_attempt_record is not None:
                self._rollback_active_attempt_record["post_attempt_action"] = (
                    "resume_base_tssfac"
                )
                self._rollback_active_attempt_record["post_attempt_message"] = (
                    resume_message
                )
        ...
        if not self._rollback_enabled or self._rollback_max_retries <= 0:
            ...
            print(
                f"{_ansys_common._LOG_PREFIX} {stop_message} "
                f"{_ansys_common._lsdyna_debug_hint(current_attempt_dir)}"
            )
        ...
        if self._rollback_current_attempt_mode == "base":
            ...
            next_rescue_depth = 1
            retry_debug_note = (
                "Streaming rollback launched a one-frame rescue attempt."
            )
        else:
            ...
            retry_debug_note = (
                "Streaming rollback launched another one-frame rescue attempt."
            )

        if self._rollback_active_attempt_record is not None:
            self._rollback_active_attempt_record["retry_reason_code"] = (
                retry_reason_code
            )
            self._rollback_active_attempt_record["retry_reason_message"] = (
                retry_reason_message
            )
            self._rollback_active_attempt_record["post_attempt_action"] = (
                "launch_rescue_retry"
        ...
        print(
            f"{_ansys_common._LOG_PREFIX} {retry_reason_message} "
            "Starting a rollback retry."
        )
    ...
    def _poll_streaming_replay(self, *, force: bool = False):
        ...
        if appended > 0:
            ...
            if self._rollback_active_attempt_record is not None:
                self._rollback_active_attempt_record[
                    "merged_frame_count_after_attempt"
                ] = int(len(self.replay.times_s))
                self._rollback_active_attempt_record["completed_full_replay"] = bool(
    ...
    def cleanup(self):
        ...
        if self._rollback_summary is not None:
            ...
            if replay_frame_count >= self._active_target_frame_count():
                ...
                self._mark_rollback_failed(
                    "Rollback example cleaned up before reaching the target replay frame count."
                )
        super().cleanup()
