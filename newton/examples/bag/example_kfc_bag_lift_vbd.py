...
def _fit_bag_contents(phys_verts_cm):
    ...
    Sphere (R=4) and box (6x6x6) are placed side-by-side in X.
...
class Example:
    ...
    def __init__(
        ...
        self._bary_disp = wp.array((full_verts_cm - bary_proj).astype(np.float32), dtype=wp.vec3)
        ...
        _non_finger_shape_indices = [s for s, b in enumerate(builder.shape_body) if b not in _finger_body_set]
        ...
        content_cfg = newton.ModelBuilder.ShapeConfig(density=0.001, mu=0.5, ke=_OBJ_SHAPE_KE, kd=_OBJ_SHAPE_KE * 1e-4)

        builder.add_body(
            xform=wp.transform(wp.vec3(s_x, s_y, s_z), wp.quat_identity()),
            mass=0.1,
        )
        builder.add_shape_sphere(body=self._robot_body_count, radius=4.0, cfg=content_cfg)

        builder.add_body(
            xform=wp.transform(wp.vec3(b_x, b_y, b_z), wp.quat_identity()),
            mass=0.1,
        )
        builder.add_shape_box(body=self._robot_body_count + 1, hx=3.0, hy=3.0, hz=3.0, cfg=content_cfg)

        builder.add_body(
            xform=wp.transform(wp.vec3(c_x, c_y, c_z), cap_quat),
            mass=0.1,
        )
        builder.add_shape_capsule(body=self._robot_body_count + 2, radius=3.0, half_height=2.0, cfg=content_cfg)
        ...
        self._proxy_indices_wp = wp.array((phys_faces.flatten() + ps).astype(np.int32), dtype=wp.int32)

        # Ground plane
        ground_cfg = newton.ModelBuilder.ShapeConfig(ke=_GROUND_SHAPE_KE, kd=0.1, mu=0.4)
        ...
        xf_np[:, :3] *= _VIZ_SCALE
        self._viz_shape_transform = wp.array(xf_np, dtype=wp.transform, device=self.model.device)

        sc_np = self.model.shape_scale.numpy().copy()
        sc_np *= _VIZ_SCALE
        self._viz_shape_scale = wp.array(sc_np, dtype=wp.vec3, device=self.model.device)
    ...
    def _set_joint_targets(self):
        ...
        if self._time_in_waypoint >= cur[1] and self._current_waypoint < len(self.waypoints) - 1:
    ...
    def test_final(self):
        ...
        assert topz > 30.0, f"Bag was not lifted: top z = {topz:.1f} cm (expected > 30 cm)"
