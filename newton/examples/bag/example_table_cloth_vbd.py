...
_SELF_CONTACT_RADIUS = 6e-3
_SELF_CONTACT_MARGIN = 1.5e-2
_REST_EXCLUSION_RADIUS = 0
_SOFT_CONTACT_MARGIN = 2.0e-2
...
# light velocity damping term removes the resulting AVBD/VBD contact jitter
# while preserving the no-pile settling behavior. This is the fraction of
# velocity retained after one second; the per-frame multiplier is derived
# from the frame time.
_CLOTH_VELOCITY_DAMPING_PER_SECOND = 0.99
...
def _damp_particle_velocities(
    ...
    damping_per_second: float,
    dt: float,
):
    i = wp.tid() + start
    if i < end:
        damping = wp.pow(damping_per_second, dt)
...
class Example:
    ...
    def __init__(
        ...
        robot_col_cfg = newton.ModelBuilder.ShapeConfig(
            ...
            kd=_SHAPE_KD,
            mu=_SHAPE_MU,
            margin=_SHAPE_MARGIN,
            # COLLIDE_SHAPES enables the viewer's "Show Collision" toggles
        ...
        if self._has_pile:
            ...
            V_pile_world = np.asarray(rigid_col_mesh.vertices, dtype=np.float64).reshape(-1, 3) @ R_pile.T + T_pile
            R_cloth = np.array(
                [
                    [
            ...
            V_cloth_world = np.asarray(vertices, dtype=np.float64).reshape(-1, 3) @ R_cloth.T + T_cloth
    ...
    def _preroll(self):
        for _ in range(_PREROLL_FRAMES):
            self.simulate(zero_velocities_each_substep=True)
        self.state_0.particle_qd.zero_()
        self.state_0.body_qd.zero_()

    def _damp_cloth_velocities(self):
        if _CLOTH_VELOCITY_DAMPING_PER_SECOND >= 1.0:
            return
        wp.launch(
            ...
            inputs=[
                ...
                _CLOTH_VELOCITY_DAMPING_PER_SECOND,
                self.frame_dt,
            ],
        )
    ...
    def _compute_cloth_metric(self, hdf5_frame: int) -> None:
        ...
        if self._replay_cloth_pos is None or self._cloth_particle_end == self._cloth_particle_start:
            return
        idx = int(min(hdf5_frame, self._replay_total_frames - 1))
