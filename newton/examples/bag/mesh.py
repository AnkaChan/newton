...
def log_mesh_counts(
    *,
    ...
    prefix = _log_prefix(log_prefix)
    print(f"{prefix}{full_label}: {len(full_verts)} verts, {len(full_faces)} {face_name}")
...
def _target_edge_length_for_face_count(
    ...
    return float(np.sqrt((4.0 * total_area) / (np.sqrt(3.0) * float(target_faces))))
...
def _cgal_isotropic_remesh(
    ...
    try:
        from CGAL import CGAL_Polygon_mesh_processing  # noqa: PLC0415
        from CGAL.CGAL_Polyhedron_3 import Polyhedron_3  # noqa: PLC0415
...
def _read_off_mesh(path: str) -> tuple[np.ndarray, np.ndarray]:
    with open(path, encoding="utf-8") as file:
        tokens = [token for line in file for token in line.split("#", maxsplit=1)[0].split()]
...
def _surface_decimate(
    ...
    try:
        ...
        raise RuntimeError("The surface-decimate proxy mode requires fast_simplification") from exc
...
def _remove_checker_reported_intersections(
    ...
    *,
    ...
    if total_removed:
        prefix = f"{log_prefix} " if log_prefix else ""
        print(
            f"{prefix}Removed {total_removed} additional proxy faces reported by the strict self-intersection checker."
        )
    return out_v, out_f
...
def _remove_edge_edge_proximity_intersections(
    ...
    *,
    ...
    if total_removed:
        prefix = f"{log_prefix} " if log_prefix else ""
        print(f"{prefix}Removed {total_removed} proxy faces to satisfy edge-edge separation >= {min_distance:.3g}.")
    return out_v, out_f
...
def _sanitize_proxy_mesh(
    ...
    *,
    ...
    unique_vertex_ids = (out_f[:, 0] != out_f[:, 1]) & (out_f[:, 1] != out_f[:, 2]) & (out_f[:, 2] != out_f[:, 0])
