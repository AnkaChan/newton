...
class Example:
    def __init__(self, viewer, video_path: str | None = None):
        ...
        cloth_size = 50
        self.left_side = [cloth_size - 1 + i * cloth_size for i in range(cloth_size)]
        self.right_side = [i * cloth_size for i in range(cloth_size)]
        ...
        self.solver = newton.solvers.SolverVBD(
            ...
            collision_detection_interval=self.collision_detection_interval,
    ...
    def render(self):
        ...
        # write video frame if requested
        if self.video_path:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--video-output", type=str, default=None, help="Optional path to write MP4 (disabled by default)"
    )

    viewer, args = newton.examples.init(parser)

    # Create example and run
    example = Example(newton.viewer.ViewerNull(num_frames=args.num_frames), video_path=args.video_output)
