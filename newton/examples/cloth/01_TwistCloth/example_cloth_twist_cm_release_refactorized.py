...
class TwistClothSimulator(Simulator):
    ...
    def step(self):
        ...
        # Debug: verify rotation t and sim_time are in sync
        # rot_t = self.t.numpy()[0]
        # print(f"sim_time={self.sim_time:.4f}, rot_t={rot_t:.4f}")
        ...
        if self.sim_time >= self.rot_end_time:
            ...
            # Modify flags and copy back to SAME array to preserve CUDA graph references
            flags = self.model.particle_flags.numpy()
            for fixed_vertex_id in self.right_side:
                flags[fixed_vertex_id] = flags[fixed_vertex_id] | ParticleFlags.ACTIVE

            # Copy into existing array instead of creating new one (CUDA graph compatibility)
            flags_new = wp.array(flags, dtype=wp.uint32, device=self.model.particle_flags.device)
            wp.copy(self.model.particle_flags, flags_new)
