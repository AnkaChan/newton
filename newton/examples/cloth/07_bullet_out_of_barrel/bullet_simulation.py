...
class BulletSimulator(Simulator):
    ...
    def custom_init(self):
        """Add bullet soft body and barrel collision mesh."""
        ...
        # Add barrel as static collision mesh
        barrel_mesh = newton.Mesh(
            self.barrel_verts,
            self.barrel_faces.flatten()
        )
    ...
    def custom_finalize(self):
        ...
        if hasattr(self, 'bullet_tets'):
            ...
            self.bullet_surface_faces = np.array(
                [face for key, (count, face) in face_count.items() if count == 1],
                dtype=np.int32
            )
            print(f"Extracted {len(self.bullet_surface_faces)} surface faces")

    def apply_bullet_force(self):
        """Apply a massive force to the bottom of the bullet (GPU kernel)."""
        wp.launch(
    ...
    def setup_polyscope_meshes(self):
        ...
        # Register barrel as surface mesh
        ps.register_surface_mesh("Barrel", self.barrel_verts, self.barrel_faces)
        
        # Register bullet
        if self.bullet_surface_faces is not None:
            bullet_verts = self.model.particle_q.numpy()[:self.bullet_vert_count]
            self.ps_bullet = ps.register_surface_mesh(
                "Bullet", bullet_verts, self.bullet_surface_faces
            )
...
if __name__ == "__main__":
    ...
    config = {
        **default_config,
        ...
        "k_lambda": 1.0e6,
        "k_damp": 1e-4,
        "particle_radius": 0.05,  # Collision radius in cm
        "force_magnitude": 5e2,
        "force_direction": [0.0, 0.0, 1.0],
        "bottom_threshold_cm": 1.5,  # Increased to capture more bottom vertices
        "color": [0.8, 0.6, 0.2],
