...
def _preview(path, vertices, faces, z_shoulder):
    ...
    for n, (name, elev, azim) in enumerate(views):
        ...
        ax.add_collection3d(
            Poly3DCollection(tris[body], facecolor="#6fa8dc", edgecolor="#274b6d", linewidths=0.12, alpha=0.85)
        )
        ax.add_collection3d(
            Poly3DCollection(tris[handle], facecolor="#e69138", edgecolor="#7a4a10", linewidths=0.2, alpha=0.98)
        )
