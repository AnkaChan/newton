...
###########################################################################
# Example VBD Bag Franka Pickup (Kinematic)
#
# A Franka FR3 arm grasps the open top of a lunch-bag-sized VBD cloth bag
# containing rigid bodies, then lifts and waves it.
#
# The robot arm is kinematic (zero inverse mass), driven by IK + FK.
# Body velocities are computed from joint velocity so VBD has correct
# friction at cloth-body contacts. VBD simulates both the cloth and
# content rigid bodies (integrate_with_external_rigid_solver=False).
# FK runs on a robot-only model copy to avoid overwriting content body
# state, eliminating the _content_body_q_save workaround.
#
# Command: python -m newton.examples vbd_bag_franka_pickup_kinematic
#
###########################################################################
...
class Example:
    ...
    def _initialize_robot_pregrasp(self):
        ...
        joint_q = self._model_single.joint_q.numpy().copy()
        joint_q[:arm_joint_count] = ik_solution[:arm_joint_count]
        for ji in self.params["gripper_joint_indices"]:
            joint_q[ji] = gripper_value

        joint_q_wp = wp.array(joint_q, dtype=float, device=self._model_single.device)
        joint_qd_wp = wp.zeros(self._model_single.joint_dof_count, dtype=float, device=self._model_single.device)

        self._state_single.joint_q.assign(joint_q_wp)
        self._state_single.joint_qd.assign(joint_qd_wp)
        newton.eval_fk(self._model_single, self._state_single.joint_q, self._state_single.joint_qd, self._state_single)

        robot_body_q = self._state_single.body_q.numpy()
        body_q = self.state_0.body_q.numpy().copy()
        body_q[: self._robot_body_count] = robot_body_q
        ...
        self._prev_robot_joint_q = joint_q.copy()
    ...
    def simulate(self):
        ...
        joint_q = self._prev_robot_joint_q.copy()
        joint_q[:arm_joint_count] = ik_solution[:arm_joint_count]
        for ji in self.params["gripper_joint_indices"]:
            joint_q[ji] = gripper_value

        joint_qd = np.zeros(self._model_single.joint_dof_count)
        joint_qd[:arm_joint_count] = (
            joint_q[:arm_joint_count] - self._prev_robot_joint_q[:arm_joint_count]
        ) / self.frame_dt
        for ji in self.params["gripper_joint_indices"]:
            joint_qd[ji] = (joint_q[ji] - self._prev_robot_joint_q[ji]) / self.frame_dt

        self._state_single.joint_q.assign(wp.array(joint_q, dtype=float, device=self._model_single.device))
        self._state_single.joint_qd.assign(wp.array(joint_qd, dtype=float, device=self._model_single.device))
        newton.eval_fk(self._model_single, self._state_single.joint_q, self._state_single.joint_qd, self._state_single)
        ...
        robot_body_qd = self._state_single.body_qd.numpy()
        body_qd = self.state_0.body_qd.numpy().copy()
        body_qd[: self._robot_body_count] = robot_body_qd
        self.state_0.body_qd.assign(wp.array(body_qd, dtype=self.state_0.body_qd.dtype, device=self.model.device))
