...
###########################################################################
# Example VBD Bag on Ground
#
# A cloth bag rests on the ground with rigid bodies inside. The top rim is
# held open to provide stable grasp points for a future Franka pickup stage,
# but this example only lets the bag and contents settle under gravity.
#
# Command: python -m newton.examples vbd_bag_on_ground
#
###########################################################################
...
class Example:
    ...
    def test_final(self):
        ...
        assert settled == len(self.info["body_indices"]), (
            f"Only {settled}/{len(self.info['body_indices'])} rigid bodies stayed in the bag"
        )
