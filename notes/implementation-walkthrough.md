...
- **Frame R_i**: the proper rotation (3x3, determinant +1) closest to the cell-center deformation gradient F_i, computed and returned detached by `closest_proper_rotations` (experiments/learned_intrinsic_solver/frames.py:4-22 describes the rule). Its columns are the world directions of the cell's local axes.
...
|---|---|---|---|
| 0-2 | rest offset | `(X_j - X_i) / h` | material rest centers X [m] over the rest edge length h [m]; on a hop-1 grid every entry is exactly -1, 0 or 1; batch independent, expanded over B |
| 3-5 | current offset in the receiver frame | `R_i^T (x_j - x_i) / h` | world cell centers x [m]; the only channel that moves with the current positions |
...
- **All-masked rows.** If a receiver had no valid slot, a row of all `-inf` would make softmax return NaN, and the NaN would flow into backward. `has_neighbor` (shape `[1, chunk, 1, 1]`) detects that case and replaces the whole row by zeros first; the following `masked_fill(~valid, 0)` then zeroes the uniform weights, so the message is zero and the token passes through the residual unchanged. The canonical topology never produces such a row (self is always valid); this is a guard for hand-built topologies and tests.
...
- `node_encoder` takes `9 + state_feature_dim` inputs: the nine entries of the current local axes A (row-major) followed by the 61-value state vector, 70 channels by default. `edge_encoder` maps the 24 raw edge channels of §1 to `edge_hidden_dim` = 64. `condition_encoder` maps the six conditioning channels to 128.
...
- Both heads start at exactly zero, weights and biases (experiments/learned_intrinsic_solver/network.py:293-296). With the default configuration the module has 323,214 parameters (pinned by a test).
...
**Input contract: the six conditioning channels.** `conditioning_channels` (experiments/learned_intrinsic_solver/features.py:348) builds one `[B, 6]` row per object from four material tensors and two Python scalars:
...
- The single-material step computes the row once at construction and registers it as a buffer (experiments/learned_intrinsic_solver/solver_step.py:168-171); the mixed-material step recomputes it per batch from each context's material and expands it to `[B, C, 6]` (experiments/learned_intrinsic_solver/mixed_physics.py:351-352).
...
where `s2(H)` is the sum of the three **principal 2x2 minors** of H (the determinants of the diagonal 2x2 blocks). The `mu tr(H)` terms cancel algebraically, leaving
...
- The same buffer also stores `boundary_features` (six exposed-face flags plus eight fixed-corner flags, `[C, 14]`) and `reference_corners`, the long `[3]` tensor of clamped-face corner IDs, or an empty tensor when the prototype has fewer than three noncollinear pins.
...
- The reference is validated before use: same dtype and device as `F`, finite, `|R_ref^T R_ref - I| <= 1e3 eps` and `det > 0` (experiments/learned_intrinsic_solver/frames.py:235-238).
...
**The least-squares problem.** The network outputs a target increment `D_c` (3x3, world coordinates, material axes in columns) for every cell. Eight cells share each interior corner, so eight increments compete for the same corner positions. Fusion resolves this by choosing the free-corner displacement `u` that best matches all increments at once. "Gauss quadrature" here is the standard eight-point rule for the cube (points at `+/- 1/sqrt(3)` along each axis, all weights equal); `dN_k/dX(q)` are the trilinear shape-function gradients at Gauss point `q`. The displacement gradient at `q` is `(grad u)_ij(q) = sum_k u_ki dN_k/dX_j(q)`. The objective is
...
- Right-hand sides must be Fortran-ordered `[N, R]` in the factor's dtype; the result is a new array of the same layout.
...
| position gradient, world axis gradient `G`, RMS, `log rms` | no | `autograd.grad` on a detached copy at experiments/learned_intrinsic_solver/input_assembly.py:177-180; NumPy solve at experiments/learned_intrinsic_solver/fusion.py:254 |
...
**Step 1: the position gradient of the physical objective.** The objective of one physical step is `E(X) = elastic(X) + inertia(X; Y) + damping(X; X_start)` in joules (§4, §5, §6), where `X` are the candidate corner positions [B, P, 3] in meters, `Y` is the inertial prediction and `X_start` the positions at the beginning of the physical step. Its gradient `g = dE/dX` has units J/m = N and is the negative of the net force on every corner, so the code calls its norm the force residual.
...
It requires both blocks to be finite `[B, C, 3, 3]` tensors in the working dtype on the module device, and the flag to be a `[B]` boolean tensor. It returns detached copies, so even a caller that hands in tensors with autograd history cannot connect two queries.
...
The consequence is that there is no backpropagation through time: the network is trained on a one-step objective (energy after this update versus before it, §7) while receiving the two-step context as data. It cannot learn to shape its history for a future query except through the positions it produces, and one query's backward pass costs one fusion adjoint solve per object, not one per query in the trajectory.
...
**The rest grid container.** `VoxelGridData` (experiments/learned_intrinsic_solver/data.py:23) is a frozen dataclass holding the shared rest geometry plus two per-cell fields that the training path never reads (`cell_deformation`, identity, and `cell_velocity`, zeros; they serve the older per-cell augmentation `augment_grid`). The fields that matter downstream:
...
**Shape.** `strength` is uniform in `strength_range`, so the multiscale budget is 2 to 10 percent of the shortest side (0.5 to 2.5 cm for the default grid). The sampled displacement `sample.positions - rest` is multiplied by `perturbation_scale`; the two exact branches keep the legacy float32 conversion bit for bit at scale 1 and return the rest grid at scale 0. Under the trainer's `(0.0, 1.0)` range the scale is continuous, so about one trajectory in ten starts with less than a tenth of the sampled multiscale displacement, which gives the network near-rest starts without a separate mode.
...
- Pool construction in `run_training` submits `capacity` (64) resets at once onto two workers; each reset costs a multiscale generation, a PARDISO factorization and a rigid predictor build, so the first batches wait on the FIFO head.
