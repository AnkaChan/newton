...
from research.principal_stretch.captured_graph_vbd import (
    CONTRACT_ID,
    DEFAULT_FIXED12_CORRECTION_KERNEL_LAUNCHES,
    DEFAULT_FIXED12_OUTER_KERNEL_LAUNCHES,
    DEFAULT_FIXED12_OUTER_KERNEL_VERSION,
    DEFAULT_FIXED12_OUTER_SCHEDULE_SHA256,
    DEFAULT_FIXED12_OUTER_SCHEDULE_VERSION,
    DEFAULT_FIXED12_SCALAR_LAUNCH_COUNTS,
    ...
    _canonical_scalar_fused_evidence,
    _derive_outer_schedule_sha256,
    _finalize_gate,
    _fused_vertex_outer_terms,
    _hash_parts,
    _initialize_from_k1,
    _outer_schedule_scalar_claims_from_evidence,
    _OuterScheduleScalarClaims,
)
...
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
    NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP as V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
)
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
...
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
    TERMINAL_COARSEST_LOGICAL_PHASES_SERIALIZED as V_CYCLE_TERMINAL_COARSEST_LOGICAL_PHASES,
)
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
...
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
    TERMINAL_FUSION_CPU_FALLBACK_ROUTE as V_CYCLE_TERMINAL_FUSION_CPU_FALLBACK_ROUTE,
)
from research.principal_stretch.correction_multigrid_warp_scalar_fused import (
...
from research.principal_stretch.solver_scenes import build_refinement_scene, build_stretch_scene
...
def _claimed_outer_schedule_sha256(claims: _OuterScheduleScalarClaims) -> str:
    """Derive the live outer digest from one complete scalar claim set."""
    return _derive_outer_schedule_sha256(
        OUTER_KERNEL_VERSION,
        FUSED_GATHER_KERNEL_VERSION,
        ROOT_SEEDED_GATHER_KERNEL_VERSION,
        SCALAR_DIRECTION_APPLY_KERNEL_VERSION,
...
def _native_cuda_graph_node_count(graph: object) -> int:
    """Count exact CUDA graph nodes from the driver's DOT representation."""
    with tempfile.TemporaryDirectory() as directory:
        path = pathlib.Path(directory) / "captured-graph.dot"
        wp.capture_debug_dot_print(graph, str(path), verbose=False)
        return sum(
            line.startswith('"graph_') and "_node_" in line and '"[' in line
...
def _coherently_rehash_scalar_physical(physical: WarpScalarFusedVCyclePhysicalWork) -> None:
    object.__setattr__(
        physical,
        "content_sha256",
        _hash_parts(
...
def _coherently_rehash_scalar_record(record: WarpScalarFusedVCycleRecord) -> None:
    object.__setattr__(
        record,
        "content_sha256",
        _hash_parts(
...
def _integrated_bit_patterns(binding) -> dict[str, tuple[str, tuple[int, ...], bytes]]:
    ...
    for outer_index, workspace in enumerate(binding.outer):
        ...
        arrays[f"outer_{outer_index}.operator_apply_delta_piola"] = workspace.operator_apply_delta_piola
        for cycle_name, cycle in (("first_cycle", workspace.first_cycle), ("second_cycle", workspace.second_cycle)):
            arrays[f"outer_{outer_index}.{cycle_name}.coarse_intermediate"] = cycle.coarse_intermediate
            for group_name in ("level_rhs", "level_correction", "level_correction_alt", "level_residual"):
                for level_index, array in enumerate(getattr(cycle, group_name)):
                    arrays[f"outer_{outer_index}.{cycle_name}.{group_name}[{level_index}]"] = array
    return {name: _device_bit_pattern(array) for name, array in arrays.items()}


def _persistent_bit_patterns(binding) -> tuple[tuple[str, tuple[str, tuple[int, ...], bytes]], ...]:
    persistent = binding.persistent
    assert persistent is not None
    return tuple((name, _device_bit_pattern(array)) for name, array in persistent.arrays)
...
class TestCapturedDirectGraphVBD(unittest.TestCase):
    def test_source_is_public_research_only_and_contains_no_krylov_solver(self):
        ...
        self.assertIn('"captured-direct-graph-vbd-graph-identity-v14"', source)
    ...
    def test_four_warp_gate_contract_and_cpu_ordered_reference_are_frozen(self):
        ...
        self.assertEqual(DEFAULT_FIXED12_OUTER_SCHEDULE_VERSION, "captured-direct-graph-vbd-outer-schedule-v14")
        self.assertEqual(
            DEFAULT_FIXED12_OUTER_SCHEDULE_SHA256,
            "2fa01a57b560c7780aa52d4c1dd933e8542a6d9d7c1579eda664f350e683447c",
        )
        self.assertEqual(DEFAULT_FIXED12_SCALAR_LAUNCH_COUNTS, (14, 13, 12))
        self.assertEqual(DEFAULT_FIXED12_OUTER_KERNEL_LAUNCHES, 32)
        self.assertEqual(DEFAULT_FIXED12_CORRECTION_KERNEL_LAUNCHES, 130)
        self.assertNotIn("OUTER_SCHEDULE_SHA256", vars(module))
...
"""
    ...
    def test_refinement_scalar_evidence_counts_routes_and_digests_scale_on_cpu(self):
        expected = {
            "coarse": {
                "levels": 4,
                "counts": (14, 13, 12, 32, 130),
                "terminal": (V_CYCLE_TERMINAL_MICROCYCLE_CUDA_ROUTE, 6, 32),
                "hashes": (
                    "aa0eb541ac8dba2bc9b41bbd564c67316a509f951c503e0b1b087ca8448d06c9",
                    "f859387a69aae3b1fe21e230e1c40840dfe21fd95ca763150afb0e52c912ecde",
                    "86a2001015917012f31588a2bb1ccdd812a41a9d9a51f3b231b4eb6d3804a3e8",
                    "dea8fbb40c174204ae5ad7a954bccd64a96caba64fbff1604206746198997122",
                ),
            },
            "medium": {
                "levels": 5,
                "counts": (20, 19, 18, 44, 178),
                "terminal": (V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE, 12, 64),
                "hashes": (
                    "2d6d0c38fca90d93eb1aecc350afb7d1a4dd71779082355803360875785e2bdf",
                    "0c03c31c2a3059fd3af716d6935f805518661301de3ffcd51c99a23a5cc59a4d",
                    "84565ca3b0b985168c9c9fae6bba79d84f3ceee772c62b1aba8b9bea1547b747",
                    "76e77616bd58b77f9a0eb4d1e5b8e40d3b4047f07a4cc02734a4ead012e6fab3",
                ),
            },
            "fine": {
                "levels": 6,
                "counts": (26, 25, 24, 56, 226),
                "terminal": (V_CYCLE_TERMINAL_MICROCYCLE_CUDA_ROUTE, 18, 96),
                "hashes": (
                    "de3b75a9b8ac02ba600b4dade8e84a84c44f1ec7c3caa7523acd681c719cdc37",
                    "0b7de5e9adc214add29e576212e9d29eed9d74d404aa914a6b24e7adbabea848",
                    "c2aaff97841b1ab23f9bcddb5db32ca1b311b6c3f91bb57c104c8d286d068054",
                    "72050b9a9918e69e7d9f816bc8071e2dee03a9c603820b916db6294eb89259d3",
                ),
            },
        }
        route_claims = []
        all_digests = []
        for level, anchors in expected.items():
            with self.subTest(level=level):
                scene = build_refinement_scene(level)
                problem = build_common_problem(scene)
                operator = MatrixFreeStableNHOperator.from_problem(problem, scene.x_current)
                ...
                evidence = _canonical_scalar_fused_evidence(hierarchy)
                claims = _outer_schedule_scalar_claims_from_evidence(evidence)
                outer_digest = _claimed_outer_schedule_sha256(claims)
                self.assertEqual(evidence.hierarchy_level_count, anchors["levels"])
                self.assertEqual(
                    (
                        evidence.scheduled_kernel_launches,
                        evidence.core_kernel_launches,
                        evidence.seeded_core_kernel_launches,
                        evidence.outer_kernel_launches_per_outer,
                        evidence.correction_kernel_launches,
                    ),
                    anchors["counts"],
                )
                self.assertEqual(
                    (
                        evidence.terminal_fusion_route,
                        evidence.terminal_coarse_scalar_size,
                        evidence.terminal_required_block_dim,
                    ),
                    anchors["terminal"],
                )
                self.assertEqual(
                    (
                        ...
                        evidence.core_schedule_sha256,
                        evidence.seeded_core_schedule_sha256,
                        outer_digest,
                    ),
                    anchors["hashes"],
                )
                route_claims.append(anchors["terminal"])
                all_digests.extend(anchors["hashes"])
        self.assertEqual(len(set(route_claims)), 3)
        self.assertEqual(len(set(all_digests)), 12)

    def test_multilevel_unit_smoothing_cannot_claim_the_b2_terminal_route(self):
        scene = build_refinement_scene("coarse")
        problem = build_common_problem(scene)
        operator = MatrixFreeStableNHOperator.from_problem(problem, scene.x_current)
        hierarchy = build_stable_nh_rest_multigrid(
            operator,
        ...
        claims = _outer_schedule_scalar_claims_from_evidence(_canonical_scalar_fused_evidence(hierarchy))
        forged = claims._replace(
            schedule_sha256="a" * 64,
            core_schedule_sha256="b" * 64,
            seeded_core_schedule_sha256="c" * 64,
            scheduled_kernel_launches=18,
            core_kernel_launches=17,
            seeded_core_kernel_launches=16,
            ...
            terminal_logical_phases=V_CYCLE_TERMINAL_B2_LOGICAL_PHASES,
            outer_kernel_launches_per_outer=40,
            correction_kernel_launches=162,
        )
        module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        with self.assertRaisesRegex(ValueError, "B2 terminal route"):
            module._require_outer_schedule_scalar_claims(forged)
    ...
    def test_diagnostic_timing_is_balanced_and_cannot_claim_performance(self):
        scalar_claims = _OuterScheduleScalarClaims(
            ...
            core_schedule_sha256="8" * 64,
            seeded_core_schedule_sha256="9" * 64,
            scheduled_kernel_launches=14,
            core_kernel_launches=13,
            seeded_core_kernel_launches=12,
            nonterminal_literal_kernel_version=V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
            nonterminal_literal_kernel_route=V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
            nonterminal_literal_physical_nodes=11,
            nonterminal_literal_physical_node_map=V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
            ...
            terminal_coarse_scalar_size=12,
            terminal_fusion_route=V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE,
            terminal_fusion_kernel_launches=1,
            terminal_fusion_launch_reduction=6,
            terminal_level_index=2,
            terminal_required_block_dim=64,
        ...
        common = {
            ...
            "v_cycle_nonterminal_literal_kernel_version": V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
            "v_cycle_nonterminal_literal_kernel_route": V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
            "v_cycle_nonterminal_literal_physical_nodes": 11,
            "v_cycle_nonterminal_literal_physical_node_map": (V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP),
            ...
            "v_cycle_terminal_coarse_solve_kernel_version": V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
            "v_cycle_terminal_coarse_solve_route": V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
            "v_cycle_terminal_coarse_scalar_size": 12,
            "v_cycle_terminal_fusion_route": V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE,
            "v_cycle_terminal_fusion_kernel_launches": 1,
            "v_cycle_terminal_fusion_launch_reduction": 6,
            "v_cycle_terminal_level_index": 2,
            "v_cycle_terminal_required_block_dim": 64,
            ...
            "v_cycle_root_ingress_zero_start_fusions": 1,
            ...
            "outer_schedule_sha256": _claimed_outer_schedule_sha256(scalar_claims),
        ...
        attacks = (
            ...
            (
                {"root_seeded_gather_kernel_version": ROOT_SEEDED_GATHER_KERNEL_VERSION + "-forged"},
            ...
            (
                {"v_cycle_nonterminal_literal_kernel_version": V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION + "-forged"},
                "nonterminal literal kernel version",
            ),
            (
                {"v_cycle_nonterminal_literal_kernel_route": V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE + "-forged"},
                "rejects non-CUDA",
            ),
            ({"v_cycle_nonterminal_literal_physical_nodes": 10}, "literal CUDA"),
            ({"v_cycle_nonterminal_literal_physical_node_map": "residual-bs3=2"}, "literal CUDA"),
            (
                {"v_cycle_terminal_fusion_version": V_CYCLE_TERMINAL_FUSION_VERSION + "-forged"},
                "terminal kernel version",
            ),
            (
                {
            ...
            (
                {
                    "v_cycle_terminal_coarse_solve_kernel_version": (
                        V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION + "-forged"
                    )
                },
                "fixed-12 terminal route",
            ),
            (
                {"v_cycle_terminal_coarse_solve_route": V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE + "-forged"},
                "fixed-12 terminal route",
            ),
            ({"v_cycle_terminal_coarse_scalar_size": 11}, "fixed-12 terminal route"),
            (
                {"v_cycle_terminal_fusion_route": V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE + "-forged"},
                "rejects CPU",
            ),
            ({"v_cycle_terminal_fusion_kernel_launches": 0}, "terminal micro-cycle"),
            ({"v_cycle_terminal_fusion_launch_reduction": 2}, "terminal micro-cycle"),
            ({"v_cycle_terminal_level_index": -1}, "requires a noncoarsest"),
            ({"v_cycle_terminal_required_block_dim": 96}, "terminal micro-cycle"),
            ({"v_cycle_terminal_block_dim": 100}, "terminal micro-cycle"),
            ({"v_cycle_terminal_collective_count": 1}, "terminal micro-cycle"),
            ({"v_cycle_terminal_owner_thread": 1}, "terminal micro-cycle"),
            ({"v_cycle_terminal_logical_phases": "forged"}, "terminal micro-cycle"),
            ({"v_cycle_hierarchy_level_count": 5}, "requires a noncoarsest"),
            ({"v_cycle_pre_smooth_steps": 2}, "terminal micro-cycle"),
            ({"v_cycle_post_smooth_steps": 2}, "terminal micro-cycle"),
            ({"v_cycle_schedule_sha256": "a" * 64}, "kernel and publication"),
            ({"v_cycle_core_schedule_sha256": "b" * 64}, "kernel and publication"),
            ({"v_cycle_seeded_core_schedule_sha256": "c" * 64}, "kernel and publication"),
            ({"v_cycle_root_ingress_zero_start_fusions": 0}, "requires a noncoarsest"),
            ({"v_cycle_seeded_root_ingress_route": V_CYCLE_ROOT_INGRESS_INTERNAL_ROUTE}, "shared root ingress"),
            ...
            ({"outer_kernel_launches_per_outer": 40}, "launch counts"),
            ({"correction_kernel_launches": 162}, "launch counts"),
        ...
        object.__setattr__(timing, "outer_schedule_version", StringSubclass(OUTER_SCHEDULE_VERSION))
        try:
            with self.assertRaisesRegex(TypeError, "exact built-in strings"):
                timing.deterministic_record()
        finally:
            object.__setattr__(timing, "outer_schedule_version", OUTER_SCHEDULE_VERSION)
        ...
        try:
            ...
            self.assertEqual(
                canonical["v_cycle_terminal_coarse_solve_kernel_version"],
                V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
            )
            self.assertEqual(
                canonical["v_cycle_terminal_coarse_solve_route"],
                V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
            )
            self.assertEqual(canonical["v_cycle_terminal_coarse_scalar_size"], 12)
            self.assertEqual(
                canonical["v_cycle_terminal_fusion_route"],
    ...
    def test_public_endpoint_constructor_cannot_mint_unvalidated_evidence(self):
        ...
        with self.assertRaisesRegex(TypeError, "_validation_context"):
            CapturedGraphVBDEndpoint(
                ...
                outer_kernel_version=OUTER_KERNEL_VERSION,
                outer_schedule_version=OUTER_SCHEDULE_VERSION,
                outer_schedule_sha256=DEFAULT_FIXED12_OUTER_SCHEDULE_SHA256,
                finalize_gate_route=FINALIZE_GATE_ROUTE,
                finalize_gate_block_dim=FINALIZE_GATE_BLOCK_DIM,
                finalize_gate_owner_threads=FINALIZE_GATE_OWNER_THREADS,
                finalize_gate_owner_roles=FINALIZE_GATE_OWNER_ROLES,
...
@unittest.skipUnless(os.environ.get("MG_VBD_TEST_CUDA") == "1", "set MG_VBD_TEST_CUDA=1 after claiming a GPU")
class TestCapturedDirectGraphVBDFixed12Cuda(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        if wp.get_cuda_device_count() < 1:
            raise unittest.SkipTest("no claimed CUDA device is visible")
    ...
    def test_oversize_terminal_topology_is_supported_and_allocates_eight_v_cycle_workspaces(self):
        scene = build_structured_cantilever_scene(
            dimensions=(7, 6, 6),
            name="captured-direct-graph-vbd-oversize-terminal",
        )
        config = DirectGraphVBDConfig(
            target_aggregate_size=500,
            minimum_aggregate_size=3,
            maximum_levels=2,
        )
        solver = CapturedDirectGraphVBD(scene, device="cuda:0", config=config)
        claims = (
            __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
            ._lookup_workspace_owners(solver)
            .claims.outer_schedule_scalar_claims
        )
        self.assertEqual(claims.terminal_fusion_route, V_CYCLE_TERMINAL_FUSION_OVERSIZE_FALLBACK_ROUTE)
        self.assertGreater(claims.terminal_required_block_dim, 1024)
        self.assertEqual(claims.terminal_block_dim, 0)
        self.assertEqual(claims.terminal_fusion_kernel_launches, 0)
        self.assertEqual(sum(2 for _workspace in solver.workspaces), 8)

    def test_generic_coarse12_non64_terminal_route_is_supported(self):
        config = DirectGraphVBDConfig(target_aggregate_size=8)
        solver = CapturedDirectGraphVBD(build_stretch_scene(), device="cuda:0", config=config)
        claims = (
            __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
            ._lookup_workspace_owners(solver)
            .claims.outer_schedule_scalar_claims
        )
        self.assertEqual(claims.terminal_fusion_route, V_CYCLE_TERMINAL_MICROCYCLE_CUDA_ROUTE)
        self.assertEqual(claims.terminal_coarse_scalar_size, 12)
        self.assertNotEqual(claims.terminal_block_dim, 64)
        self.assertEqual(sum(2 for _workspace in solver.workspaces), 8)

    def test_b2_terminal_route_is_supported_for_nonunit_smoothing(self):
        solver = CapturedDirectGraphVBD(
            build_stretch_scene(),
            device="cuda:0",
            config=DirectGraphVBDConfig(pre_smooth_steps=2, post_smooth_steps=2),
        )
        claims = (
            __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
            ._lookup_workspace_owners(solver)
        ...
        self.assertEqual(claims.terminal_fusion_route, V_CYCLE_TERMINAL_FUSION_CUDA_ROUTE)
        self.assertEqual(
            (
                claims.scheduled_kernel_launches,
                claims.core_kernel_launches,
                claims.seeded_core_kernel_launches,
            ),
            (30, 29, 28),
        )
        self.assertEqual(claims.outer_kernel_launches_per_outer, 64)
        self.assertEqual(claims.correction_kernel_launches, 258)

    def test_cpu_coarsest_unknown_and_corrupt_claims_reject_before_v_cycle_workspace_allocation(self):
        module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        original_projector = module._outer_schedule_scalar_claims_from_evidence
        attacks = {
            "cpu": {
                "terminal_fusion_route": V_CYCLE_TERMINAL_FUSION_CPU_FALLBACK_ROUTE,
                ...
                "terminal_logical_phases": V_CYCLE_TERMINAL_LEGACY_LOGICAL_PHASES,
            },
            "coarsest": {
                ...
                "terminal_collective_count": 0,
                "terminal_owner_thread": -1,
                "terminal_logical_phases": V_CYCLE_TERMINAL_COARSEST_LOGICAL_PHASES,
                "root_ingress_zero_start_fusions": 0,
                "seeded_root_ingress_route": V_CYCLE_ROOT_INGRESS_COARSE_COPY_ROUTE,
            },
            "unknown": {"terminal_fusion_route": "cuda-unknown-terminal-route-v1"},
            "corrupt": {"schedule_sha256": "f" * 63},
        }
        for name, mutations in attacks.items():
            with self.subTest(attack=name):

                def reject_projected_claims(evidence, *, _mutations=mutations):
                    claims = original_projector(evidence)._replace(**_mutations)
                    return module._require_outer_schedule_scalar_claims(claims)

                with (
                    mock.patch.object(
                        module,
                        "_outer_schedule_scalar_claims_from_evidence",
                        side_effect=reject_projected_claims,
                    ),
                    mock.patch.object(
                        WarpScalarFusedStaticMultigridHierarchy,
                        "create_workspace",
                        side_effect=AssertionError("V-cycle workspace allocated before schedule rejection"),
                    ) as create_workspace,
                    self.assertRaisesRegex(ValueError, "canonical CUDA scalar terminal route"),
                ):
                    CapturedDirectGraphVBD(build_stretch_scene(), device="cuda:0")
                self.assertEqual(create_workspace.call_count, 0)

    def test_static_device_corruption_rejects_before_v_cycle_workspace_allocation(self):
        original_from_device_hierarchy = WarpScalarFusedStaticMultigridHierarchy.from_device_hierarchy

        def corrupt_static_device_content(source_hierarchy):
            scalar_hierarchy = original_from_device_hierarchy(source_hierarchy)
            scalar_hierarchy.coarse_cholesky.fill_(0.0)
            return scalar_hierarchy

        with (
            mock.patch.object(
                WarpScalarFusedStaticMultigridHierarchy,
                "from_device_hierarchy",
                side_effect=corrupt_static_device_content,
            ),
            mock.patch.object(
                WarpScalarFusedStaticMultigridHierarchy,
                "create_workspace",
                side_effect=AssertionError("V-cycle workspace allocated before static-content rejection"),
            ) as create_workspace,
            self.assertRaisesRegex(RuntimeError, "scalar_hierarchy.static_array_0"),
        ):
            CapturedDirectGraphVBD(build_stretch_scene(), device="cuda:0")
        self.assertEqual(create_workspace.call_count, 0)
    ...
    def test_exact_two_cycle_work_and_launch_count_are_retained(self):
        ...
        logical_core_launches = self.solver.device_hierarchy.core_kernel_launches
        seeded_core_launches = self.solver.device_hierarchy.seeded_core_kernel_launches
        self.assertEqual(seeded_core_launches, logical_core_launches - 1)
        expected_linear = 5 + 2 * seeded_core_launches
        ...
        for outer_index, work in enumerate(endpoint.outer_work):
            with self.subTest(outer=outer_index):
                ...
                for record in work.v_cycles:
                    ...
                    expected_cycle_launches = seeded_core_launches
                    expected_route = V_CYCLE_EXTERNAL_SHARED_PUBLICATION_ROUTE
                    expected_schedule = self.solver.device_hierarchy.seeded_core_schedule_sha256
                    ...
                    self.assertEqual(physical.core_kernel_launches, seeded_core_launches)
                    ...
                    self.assertEqual(physical.noncoarse_level_count, len(record.work.level_visits) - 1)
                    self.assertEqual(
                        physical.nonterminal_literal_kernel_version,
                        V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
                    )
                    self.assertEqual(
                        physical.nonterminal_literal_kernel_route,
                        V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
                    )
                    self.assertEqual(physical.nonterminal_literal_physical_nodes, 11)
                    self.assertEqual(
                        physical.nonterminal_literal_physical_node_map,
                        V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
                    )
                    self.assertEqual(physical.root_ingress_zero_start_fusions, 1)
                    self.assertEqual(physical.root_ingress_route, V_CYCLE_ROOT_INGRESS_EXTERNAL_SHARED_ROUTE)
                    ...
                    self.assertEqual(
                        physical.terminal_coarse_solve_kernel_version,
                        V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
                    )
                    self.assertEqual(
                        physical.terminal_coarse_solve_route,
                        V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
                    )
                    self.assertEqual(physical.terminal_coarse_scalar_size, 12)
                    self.assertEqual(
                        physical.terminal_fusion_route,
                        V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE,
                    )
        ...
        self.assertEqual(schedule["graph_identity_schema"], "captured-direct-graph-vbd-graph-identity-v14")
        ...
        self.assertEqual(
            schedule["v_cycle_nonterminal_literal_kernel_version"],
            V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
        )
        self.assertEqual(
            schedule["v_cycle_nonterminal_literal_kernel_route"],
            V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
        )
        self.assertEqual(schedule["v_cycle_nonterminal_literal_physical_nodes"], 11)
        self.assertEqual(
            schedule["v_cycle_nonterminal_literal_physical_node_map"],
            V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
        )
        ...
        self.assertEqual(
            schedule["v_cycle_terminal_coarse_solve_kernel_version"],
            V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
        )
        self.assertEqual(
            schedule["v_cycle_terminal_coarse_solve_route"],
            V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
        )
        self.assertEqual(schedule["v_cycle_terminal_coarse_scalar_size"], 12)
        self.assertEqual(
            schedule["v_cycle_terminal_fusion_route"],
            V_CYCLE_TERMINAL_MICROCYCLE_FIXED12_CUDA_ROUTE,
        )
        ...
        self.assertEqual(schedule["separate_root_ingress_kernel_launches_per_outer"], 0)
        ...
        self.assertEqual(schedule["outer_schedule_sha256"], self.solver._outer_schedule_sha256_bound)
        ...
        self.assertEqual(schedule["v_cycle_seeded_root_ingress_route"], V_CYCLE_ROOT_INGRESS_EXTERNAL_SHARED_ROUTE)
        ...
        self.assertEqual(endpoint_record["root_seeded_gather_kernel_version"], ROOT_SEEDED_GATHER_KERNEL_VERSION)
        self.assertEqual(
        ...
        self.assertEqual(
            endpoint_record["v_cycle_nonterminal_literal_kernel_version"],
            V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
        )
        self.assertEqual(
            endpoint_record["v_cycle_nonterminal_literal_kernel_route"],
            V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
        )
        self.assertEqual(endpoint_record["v_cycle_nonterminal_literal_physical_nodes"], 11)
        self.assertEqual(
            endpoint_record["v_cycle_nonterminal_literal_physical_node_map"],
            V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
        )
        ...
        self.assertEqual(endpoint_record["outer_schedule_sha256"], self.solver._outer_schedule_sha256_bound)
        ...
        self.assertEqual(
            endpoint_record["v_cycle_terminal_coarse_solve_kernel_version"],
            V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
        )
        self.assertEqual(
            endpoint_record["v_cycle_terminal_coarse_solve_route"],
            V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
        )
        self.assertEqual(endpoint_record["v_cycle_terminal_coarse_scalar_size"], 12)
        self.assertEqual(
        ...
        self.assertEqual(endpoint_record["execution_authentication"], EXECUTION_AUTHENTICATION)
        self.assertFalse(endpoint_record["solver_issued_authentication"])
        self.assertFalse(endpoint_record["performance_evidence"])
        for outer_record in endpoint_record["outer_work"]:
            ...
            self.assertEqual(outer_record["root_seeded_gather_kernel_version"], ROOT_SEEDED_GATHER_KERNEL_VERSION)
            self.assertEqual(
            ...
            self.assertEqual(
                outer_record["v_cycle_nonterminal_literal_kernel_version"],
                V_CYCLE_NONTERMINAL_LITERAL_KERNEL_VERSION,
            )
            self.assertEqual(
                outer_record["v_cycle_nonterminal_literal_kernel_route"],
                V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
            )
            self.assertEqual(outer_record["v_cycle_nonterminal_literal_physical_nodes"], 11)
            self.assertEqual(
                outer_record["v_cycle_nonterminal_literal_physical_node_map"],
                V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
            )
            ...
            self.assertEqual(outer_record["outer_schedule_sha256"], self.solver._outer_schedule_sha256_bound)
            ...
            self.assertEqual(
                outer_record["v_cycle_terminal_coarse_solve_kernel_version"],
                V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
            )
            self.assertEqual(
                outer_record["v_cycle_terminal_coarse_solve_route"],
                V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
            )
            self.assertEqual(outer_record["v_cycle_terminal_coarse_scalar_size"], 12)
            self.assertEqual(
            ...
            self.assertEqual(outer_record["execution_authentication"], EXECUTION_AUTHENTICATION)
            self.assertFalse(outer_record["solver_issued_authentication"])
            self.assertFalse(outer_record["performance_evidence"])
            for v_cycle_record in outer_record["v_cycles"]:
                self.assertEqual(
                    v_cycle_record["physical_execution_authentication"],
                    V_CYCLE_PHYSICAL_EXECUTION_AUTHENTICATION,
                )
                self.assertFalse(v_cycle_record["solver_issued_authentication"])
                self.assertFalse(v_cycle_record["performance_evidence"])
        json.dumps(endpoint_record, allow_nan=False)

    def test_integrated_launch_trace_has_exactly_eleven_literal_substitutions_per_tail_and_88_total(self):
        scalar_module = __import__(
            "research.principal_stretch.correction_multigrid_warp_scalar_fused",
            fromlist=["*"],
        )
        graph_module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        solver = CapturedDirectGraphVBD(self.scene, device="cuda:0")
        binding = graph_module._lookup_workspace_owners(solver)
        ...
        expected_per_tail = (
            (scalar_module._scalar_csr_residual_bs3, 2),
            (scalar_module._scalar_csr_residual_bs6, 2),
            (scalar_module._zero_start_scalar_jacobi_bs6, 1),
            (scalar_module._out_of_place_scalar_jacobi_bs3, 1),
            (scalar_module._out_of_place_scalar_jacobi_bs6, 1),
            (scalar_module._restrict_owned_rows_3to6, 1),
            (scalar_module._restrict_owned_rows_6to6, 1),
            (scalar_module._prolong_add_owned_rows_3from6, 1),
            (scalar_module._prolong_add_owned_rows_6from6, 1),
        )
        tail_count = OUTER_CORRECTIONS * V_CYCLES_PER_OUTER
        for kernel, count in expected_per_tail:
            self.assertEqual(kernels.count(kernel), count * tail_count, str(kernel))
        self.assertEqual(sum(kernels.count(kernel) for kernel, _count in expected_per_tail), 88)
        schedule = solver.deterministic_record()
        self.assertEqual(schedule["v_cycle_nonterminal_literal_physical_nodes"], 11)
        self.assertEqual(
            schedule["v_cycle_nonterminal_literal_physical_node_map"],
            V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP,
        )
        self.assertEqual(schedule["literal_kernel_substitutions_per_outer"], 22)
        self.assertEqual(schedule["literal_kernel_substitutions_per_integrated_correction"], 88)
    ...
    def test_130_route_is_bitwise_equal_to_exact_committed_162_route(self):
        for label, config in (
            ...
            with self.subTest(route=label):
                ...
                wp.capture_launch(committed_capture.graph)
                fused_replay = _integrated_bit_patterns(fused_binding)
                committed_replay = _integrated_bit_patterns(committed_binding)
                self.assertEqual(fused_replay.keys(), committed_replay.keys())
                for name in fused_replay:
                    with self.subTest(route=label, replay_array=name):
                        self.assertEqual(fused_replay[name], committed_replay[name])

    def test_literal_and_fixed12_integrated_direct_capture_are_all464_bitwise_generic(self):
        scalar_module = __import__(
            "research.principal_stretch.correction_multigrid_warp_scalar_fused",
            fromlist=["*"],
        )
        generic_kernels = {
            "_scalar_csr_residual_bs3": scalar_module._scalar_csr_residual,
            "_scalar_csr_residual_bs6": scalar_module._scalar_csr_residual,
            "_zero_start_scalar_jacobi_bs6": scalar_module._zero_start_scalar_jacobi,
            "_out_of_place_scalar_jacobi_bs3": scalar_module._out_of_place_scalar_jacobi,
            "_out_of_place_scalar_jacobi_bs6": scalar_module._out_of_place_scalar_jacobi,
            "_restrict_owned_rows_3to6": scalar_module._restrict_owned_rows,
            "_restrict_owned_rows_6to6": scalar_module._restrict_owned_rows,
            "_prolong_add_owned_rows_3from6": scalar_module._prolong_add_owned_rows,
            "_prolong_add_owned_rows_6from6": scalar_module._prolong_add_owned_rows,
            "_terminal_zero_jacobi_residual_restrict_fixed12_solve_prolong_residual_jacobi": (
                scalar_module._terminal_zero_jacobi_residual_restrict_solve_prolong_residual_jacobi
            ),
        }
        graph_module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        for label, config in (
            ...
            for order in ("fixed-generic", "generic-fixed"):
                with self.subTest(route=label, order=order):
                    fixed = CapturedDirectGraphVBD(self.scene, device="cuda:0", config=config)
                    generic = CapturedDirectGraphVBD(self.scene, device="cuda:0", config=config)
                    fixed_binding = graph_module._lookup_workspace_owners(fixed)
                    generic_binding = graph_module._lookup_workspace_owners(generic)
                    fixed.poison(seed=186011)
                    generic.poison(seed=186011)
                    for (fixed_name, fixed_array), (generic_name, generic_array) in zip(
                        fixed_binding.persistent.arrays,
                        generic_binding.persistent.arrays,
                        strict=True,
                    ):
                        self.assertEqual(fixed_name, generic_name)
                        wp.copy(generic_array, fixed_array)

                    def enqueue_fixed(fixed=fixed, fixed_binding=fixed_binding) -> None:
                        fixed._enqueue_integrated(fixed_binding)

                    def enqueue_generic(generic=generic, generic_binding=generic_binding) -> None:
                        with mock.patch.multiple(scalar_module, **generic_kernels):
                            generic._enqueue_integrated(generic_binding)

                    first, second = (
                        (enqueue_fixed, enqueue_generic)
                        if order == "fixed-generic"
                        else (enqueue_generic, enqueue_fixed)
                    )
                    first()
                    second()
                    fixed_patterns = _integrated_bit_patterns(fixed_binding)
                    generic_patterns = _integrated_bit_patterns(generic_binding)
                    self.assertEqual(fixed_patterns, generic_patterns)
                    fixed_persistent = _persistent_bit_patterns(fixed_binding)
                    generic_persistent = _persistent_bit_patterns(generic_binding)
                    self.assertEqual(len(fixed_persistent), 464)
                    self.assertEqual(fixed_persistent, generic_persistent)

                    with wp.ScopedCapture(device=fixed.device) as fixed_capture:
                        fixed._enqueue_integrated(fixed_binding)
                    with (
                        mock.patch.multiple(scalar_module, **generic_kernels),
                        wp.ScopedCapture(device=generic.device) as generic_capture,
                    ):
                        generic._enqueue_integrated(generic_binding)
                    fixed.poison(seed=186012)
                    generic.poison(seed=186012)
                    wp.capture_launch(fixed_capture.graph)
                    wp.capture_launch(generic_capture.graph)
                    fixed_replay = _integrated_bit_patterns(fixed_binding)
                    generic_replay = _integrated_bit_patterns(generic_binding)
                    self.assertEqual(fixed_replay, generic_replay)
                    fixed_persistent_replay = _persistent_bit_patterns(fixed_binding)
                    generic_persistent_replay = _persistent_bit_patterns(generic_binding)
                    self.assertEqual(len(fixed_persistent_replay), 464)
                    self.assertEqual(fixed_persistent_replay, generic_persistent_replay)
    ...
    def test_coordinated_outer_schedule_version_and_hash_tamper_fails_closed(self):
        ...
        original_root_seeded_version = module.ROOT_SEEDED_GATHER_KERNEL_VERSION
        original_root_seeded_facade = self.solver._root_seeded_gather_kernel_version_bound
        scalar_claims = module._lookup_workspace_owners(self.solver).claims.outer_schedule_scalar_claims
        ...
        for boundary_name, operation in self._owner_boundaries(self.solver):
            with self.subTest(boundary=boundary_name):
                try:
                    ...
                    module.ROOT_SEEDED_GATHER_KERNEL_VERSION = original_root_seeded_version + "-forged"
                    ...
                    module.FINALIZE_GATE_ROUTE = original_globals[6] + "-forged"
                    module.FINALIZE_GATE_BLOCK_DIM = 96
                    module.FINALIZE_GATE_OWNER_THREADS = (0, 1, 2, 3)
                    module.FINALIZE_GATE_OWNER_ROLES = tuple(role + "-forged" for role in original_globals[9])
                    module.FINALIZE_GATE_COLLECTIVE_VERSION = original_globals[10] + "-forged"
                    forged_sha256 = module._derive_outer_schedule_sha256(
                        module.OUTER_KERNEL_VERSION,
                        module.FUSED_GATHER_KERNEL_VERSION,
                        module.ROOT_SEEDED_GATHER_KERNEL_VERSION,
                        module.SCALAR_DIRECTION_APPLY_KERNEL_VERSION,
                    ...
                    self.solver._root_seeded_gather_kernel_version_bound = module.ROOT_SEEDED_GATHER_KERNEL_VERSION
                    self.solver._scalar_direction_apply_kernel_version_bound = (
                        module.SCALAR_DIRECTION_APPLY_KERNEL_VERSION
                    )
                    ...
                    module.ROOT_SEEDED_GATHER_KERNEL_VERSION = original_root_seeded_version
                    self.solver._root_seeded_gather_kernel_version_bound = original_root_seeded_facade
    ...
    def test_endpoint_and_outer_serializers_revalidate_context_and_content_hashes(self):
        ...
        original_context_schedule = (
            ...
            context.root_seeded_gather_kernel_version,
            ...
            context.v_cycle_seeded_root_ingress_route,
        ...
        forged_root_seeded_version = original_context_schedule[1] + "-forged"
        forged_scalar_apply_version = original_context_schedule[2] + "-forged"
        forged_publication_version = original_context_schedule[3] + "-forged"
        forged_standalone_route = original_context_schedule[4] + "-forged"
        forged_external_route = original_context_schedule[5] + "-forged"
        forged_seeded_root_route = original_context_schedule[6] + "-forged"
        forged_first_role = original_context_schedule[7] + "-forged"
        forged_second_role = original_context_schedule[8] + "-forged"
        forged_kernel_version = original_context_schedule[9] + "-forged"
        forged_schedule_version = original_context_schedule[10] + "-forged"
        module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        forged_sha256 = module._derive_outer_schedule_sha256(
            forged_kernel_version,
            forged_gather_version,
            forged_root_seeded_version,
            forged_scalar_apply_version,
            context.v_cycle_kernel_version,
            context.v_cycle_schedule_version,
            module._outer_schedule_scalar_claims_from_context(context),
        ...
        for name, serializer in serializers:
            with self.subTest(context=name):
                try:
                    ...
                    object.__setattr__(context, "root_seeded_gather_kernel_version", forged_root_seeded_version)
                    object.__setattr__(
                        context,
                        "scalar_direction_apply_kernel_version",
                        forged_scalar_apply_version,
                    ...
                    object.__setattr__(context, "v_cycle_seeded_root_ingress_route", forged_seeded_root_route)
                    ...
                    object.__setattr__(context, "root_seeded_gather_kernel_version", original_context_schedule[1])
                    object.__setattr__(
                        context,
                        "scalar_direction_apply_kernel_version",
                        original_context_schedule[2],
                    )
                    object.__setattr__(context, "v_cycle_publication_version", original_context_schedule[3])
                    object.__setattr__(context, "v_cycle_standalone_publication_route", original_context_schedule[4])
                    object.__setattr__(
                        context, "v_cycle_external_shared_publication_route", original_context_schedule[5]
                    )
                    object.__setattr__(context, "v_cycle_seeded_root_ingress_route", original_context_schedule[6])
                    object.__setattr__(context, "first_cycle_publication_role", original_context_schedule[7])
                    object.__setattr__(context, "second_cycle_publication_role", original_context_schedule[8])
                    object.__setattr__(context, "outer_kernel_version", original_context_schedule[9])
                    object.__setattr__(context, "outer_schedule_version", original_context_schedule[10])
                    object.__setattr__(context, "outer_schedule_sha256", original_context_schedule[11])
    ...
    def test_outer_and_endpoint_policy_is_same_false_claim_for_both_routes(self):
        replay = self.solver.run(graph_replay=True)
        direct = self.solver.run(graph_replay=False)
        self.assertTrue(replay.graph_replay)
        self.assertFalse(direct.graph_replay)
        self.assertNotEqual(replay.graph_identity_sha256, direct.graph_identity_sha256)
        for endpoint in (replay, direct):
            with self.subTest(graph_replay=endpoint.graph_replay):
                evidence_values = (endpoint, *endpoint.outer_work)
                for evidence in evidence_values:
                    self.assertEqual(evidence.execution_authentication, EXECUTION_AUTHENTICATION)
                    self.assertFalse(evidence.solver_issued_authentication)
                    self.assertFalse(evidence.performance_evidence)
                    rebuilt = dataclasses.replace(evidence)
                    self.assertEqual(rebuilt.execution_authentication, EXECUTION_AUTHENTICATION)
                    self.assertFalse(rebuilt.solver_issued_authentication)
                    self.assertFalse(rebuilt.performance_evidence)
                    for field_name, forged_value in (
                        ("execution_authentication", "caller-selected-authentication"),
                        ("solver_issued_authentication", True),
                        ("performance_evidence", True),
                    ):
                        with self.subTest(type=type(evidence).__name__, field=field_name):
                            with self.assertRaisesRegex(ValueError, "init=False"):
                                dataclasses.replace(evidence, **{field_name: forged_value})

                serialized = endpoint.deterministic_record()
                self.assertEqual(serialized["execution_authentication"], EXECUTION_AUTHENTICATION)
                self.assertFalse(serialized["solver_issued_authentication"])
                self.assertFalse(serialized["performance_evidence"])
                for outer in serialized["outer_work"]:
                    self.assertEqual(outer["execution_authentication"], EXECUTION_AUTHENTICATION)
                    self.assertFalse(outer["solver_issued_authentication"])
                    self.assertFalse(outer["performance_evidence"])

    def test_authentication_tamper_with_coordinated_rehash_is_rejected(self):
        module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        canonical_digest = module._canonical_digest
        replay = self.solver.run(graph_replay=True)
        direct = self.solver.run(graph_replay=False)
        for endpoint in (replay, direct):
            for evidence, contract, hash_field in (
                (endpoint.outer_work[0], "captured-direct-graph-vbd-outer-work-v12", "content_sha256"),
                (endpoint, CONTRACT_ID, "endpoint_sha256"),
            ):
                payloads = []

                def capturing_digest(value, captured_payloads=payloads):
                    if type(value) is dict:
                        captured_payloads.append(dict(value))
                    return canonical_digest(value)

                with mock.patch.object(module, "_canonical_digest", side_effect=capturing_digest):
                    dataclasses.replace(evidence)
                payload = next(value for value in payloads if value.get("contract") == contract)
                original_hash = getattr(evidence, hash_field)
                for field_name, forged_value in (
                    ("execution_authentication", "solver-launch-authenticated-v1"),
                    ("solver_issued_authentication", True),
                    ("performance_evidence", True),
                ):
                    with self.subTest(
                        graph_replay=endpoint.graph_replay,
                        type=type(evidence).__name__,
                        field=field_name,
                    ):
                        forged_payload = dict(payload)
                        forged_payload[field_name] = forged_value
                        original_value = getattr(evidence, field_name)
                        try:
                            object.__setattr__(evidence, field_name, forged_value)
                            object.__setattr__(evidence, hash_field, canonical_digest(forged_payload))
                            with self.assertRaisesRegex(ValueError, "content hash is not canonical"):
                                evidence.deterministic_record()
                        finally:
                            object.__setattr__(evidence, field_name, original_value)
                            object.__setattr__(evidence, hash_field, original_hash)

    def test_replay_and_direct_contexts_cannot_transplant(self):
        replay = self.solver.run(graph_replay=True)
        direct = self.solver.run(graph_replay=False)
        for source, target in ((replay, direct), (direct, replay)):
            with self.subTest(source_replay=source.graph_replay, target_replay=target.graph_replay):
                with self.assertRaisesRegex(
                    ValueError,
                    "graph identity|graph provenance|another endpoint|schedule slot",
                ):
                    dataclasses.replace(source, _validation_context=target._validation_context)
                with self.assertRaisesRegex(ValueError, "schedule slot|execution context"):
                    dataclasses.replace(
                        source.outer_work[0],
                        _validation_context=target._validation_context,
                    )

    def test_noop_direct_enqueue_and_replay_both_remain_unauthenticated(self):
        direct_before = self.solver.run(graph_replay=False)
        self.assertEqual(direct_before.execution_authentication, EXECUTION_AUTHENTICATION)
        self.assertFalse(direct_before.solver_issued_authentication)
        with mock.patch.object(self.solver, "_enqueue_integrated") as enqueue:
            direct_after = self.solver.run(graph_replay=False)
        enqueue.assert_called_once()
        self.assertEqual(direct_after.execution_authentication, EXECUTION_AUTHENTICATION)
        self.assertFalse(direct_after.solver_issued_authentication)
        self.assertFalse(direct_after.performance_evidence)
        replay = self.solver.run(graph_replay=True)
        self.assertEqual(replay.execution_authentication, EXECUTION_AUTHENTICATION)
        self.assertFalse(replay.solver_issued_authentication)
        self.assertFalse(replay.performance_evidence)

    def test_execution_receipt_route_cannot_change_after_issuance(self):
        original_record = self.solver._record
        for issued_graph_replay in (False, True):
            intercepted = {}

            def intercept_record(*, execution_receipt, owner_binding=None, destination=intercepted):
                destination["receipt"] = execution_receipt
                destination["owner_binding"] = owner_binding
                return self.endpoint

            with self.subTest(issued_graph_replay=issued_graph_replay):
                with mock.patch.object(self.solver, "_record", side_effect=intercept_record):
                    self.solver.run(graph_replay=issued_graph_replay)
                receipt = intercepted["receipt"]
                owner_binding = intercepted["owner_binding"]
                object.__setattr__(receipt, "graph_replay", not issued_graph_replay)
                object.__setattr__(
                    receipt,
                    "capture_binding",
                    None if issued_graph_replay else owner_binding.capture,
                )
                with self.assertRaisesRegex(RuntimeError, "sequencing registration"):
                    original_record(execution_receipt=receipt, owner_binding=owner_binding)

    def test_coherent_receipt_route_rewrite_can_only_remain_unauthenticated(self):
        original_record = self.solver._record
        intercepted = {}

        def intercept_record(*, execution_receipt, owner_binding=None):
            intercepted["receipt"] = execution_receipt
            intercepted["owner_binding"] = owner_binding
            return self.endpoint

        with mock.patch.object(self.solver, "_record", side_effect=intercept_record):
            self.solver.run(graph_replay=False)
        receipt = intercepted["receipt"]
        owner_binding = intercepted["owner_binding"]
        object.__setattr__(receipt, "graph_replay", True)
        object.__setattr__(receipt, "capture_binding", owner_binding.capture)
        self.solver._issued_execution_receipts[id(receipt)] = (
            receipt,
            self.solver,
            receipt.serial,
            True,
            owner_binding.capture,
        )
        rewritten = original_record(execution_receipt=receipt, owner_binding=owner_binding)
        self.assertTrue(rewritten.graph_replay)
        self.assertEqual(rewritten.graph_identity_sha256, self.solver.graph_identity_sha256)
        self.assertEqual(rewritten.execution_authentication, EXECUTION_AUTHENTICATION)
        self.assertFalse(rewritten.solver_issued_authentication)
        self.assertFalse(rewritten.performance_evidence)
        for outer in rewritten.outer_work:
            self.assertEqual(outer.execution_authentication, EXECUTION_AUTHENTICATION)
            self.assertFalse(outer.solver_issued_authentication)
            self.assertFalse(outer.performance_evidence)
    ...
    def test_coordinated_rehash_cannot_forge_candidate_or_negative_vcycle_work(self):
        ...
        record_sha256 = _hash_parts(
            ...
            (
                ...
                (
                    ...
                    record.standalone_device_snapshot_sha256,
                ),
                ("core_device_snapshot_sha256", record.core_device_snapshot_sha256),
                (
                    "seeded_core_device_snapshot_sha256",
                    record.seeded_core_device_snapshot_sha256,
                ),
        ...
        physical_sha256 = _hash_parts(
            ...
            (
                ...
                ("physical_execution_authentication", physical.physical_execution_authentication),
                ("solver_issued_authentication", physical.solver_issued_authentication),
                ("performance_evidence", physical.performance_evidence),
            ),
        )
        ...
        original_root_route = physical.root_ingress_route
        original_root_launches = physical.root_ingress_kernel_launches
        try:
            ...
            object.__setattr__(record, "content_sha256", forged_record_sha256)
            with self.assertRaisesRegex(ValueError, "physical root_ingress_route"):
                dataclasses.replace(outer)
        finally:
            object.__setattr__(physical, "root_ingress_route", original_root_route)
            object.__setattr__(physical, "root_ingress_kernel_launches", original_root_launches)
            object.__setattr__(physical, "content_sha256", original_physical_sha256)
            object.__setattr__(record, "content_sha256", original_record_sha256)
        ...
        original_seeded_core_schedule = context.v_cycle_seeded_core_schedule_sha256
        ...
        original_seeded_core_snapshot = context.v_cycle_seeded_core_device_snapshot_sha256
        ...
        original_terminal = {
            ...
            "v_cycle_nonterminal_literal_physical_node_map": context.v_cycle_nonterminal_literal_physical_node_map,
            ...
            "v_cycle_terminal_coarse_solve_kernel_version": context.v_cycle_terminal_coarse_solve_kernel_version,
        ...
        original_root_route = context.v_cycle_seeded_root_ingress_route
        original_capture_binding = context.capture_binding
        try:
            object.__setattr__(context, "v_cycle_schedule_sha256", "c" * 64)
            with self.assertRaisesRegex(
                ValueError,
                "outer scalar schedule claims|scalar-fused schedule identity",
            ):
            ...
            with self.assertRaisesRegex(ValueError, "outer scalar schedule claims|core schedule identity"):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_core_schedule_sha256", original_core_schedule)
            object.__setattr__(context, "v_cycle_seeded_core_schedule_sha256", "b" * 64)
            with self.assertRaisesRegex(ValueError, "outer scalar schedule claims|seeded-core schedule identity"):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_seeded_core_schedule_sha256", original_seeded_core_schedule)
            ...
            object.__setattr__(context, "v_cycle_seeded_core_device_snapshot_sha256", "9" * 64)
            with self.assertRaisesRegex(ValueError, "seeded-core device snapshot identity"):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_seeded_core_device_snapshot_sha256", original_seeded_core_snapshot)
            object.__setattr__(context, "v_cycle_kernel_launches", original_launches + 1)
            with self.assertRaisesRegex(ValueError, "outer scalar schedule claims|launch counts?|V-cycle launch count"):
                dataclasses.replace(self.endpoint)
            ...
            with self.assertRaisesRegex(
                ValueError,
                "outer scalar schedule claims|launch counts?|full/core launch counts|core launch count",
            ):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_core_kernel_launches", original_core_launches)
            object.__setattr__(context, "v_cycle_seeded_core_kernel_launches", original_seeded_core_launches + 1)
            with self.assertRaisesRegex(
                ValueError,
                "outer scalar schedule claims|launch counts?|seeded V-cycle core launch count|seeded V-cycle core",
            ):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_seeded_core_kernel_launches", original_seeded_core_launches)
            terminal_attacks = {
                "v_cycle_nonterminal_literal_kernel_version": (
                    original_terminal["v_cycle_nonterminal_literal_kernel_version"] + "-forged"
                ),
                "v_cycle_nonterminal_literal_kernel_route": (
                    original_terminal["v_cycle_nonterminal_literal_kernel_route"] + "-forged"
                ),
                "v_cycle_nonterminal_literal_physical_nodes": 10,
                "v_cycle_nonterminal_literal_physical_node_map": "residual-bs3=2",
                ...
                "v_cycle_terminal_coarse_solve_kernel_version": (
                    original_terminal["v_cycle_terminal_coarse_solve_kernel_version"] + "-forged"
                ),
                "v_cycle_terminal_coarse_solve_route": (
                    original_terminal["v_cycle_terminal_coarse_solve_route"] + "-forged"
                ),
                "v_cycle_terminal_coarse_scalar_size": 11,
                ...
                "v_cycle_terminal_required_block_dim": (original_terminal["v_cycle_terminal_required_block_dim"] + 32),
            ...
            for field_name, forged_value in terminal_attacks.items():
                with self.subTest(context_terminal_field=field_name):
                    object.__setattr__(context, field_name, forged_value)
                    with self.assertRaisesRegex(
                        ValueError,
                        "captured outer schedule|outer scalar schedule claims|literal|terminal fusion|"
                        "terminal V-cycle|outer kernel schedule",
                    ):
                        dataclasses.replace(self.endpoint)
                    object.__setattr__(context, field_name, original_terminal[field_name])
            ...
            with self.assertRaisesRegex(ValueError, "outer scalar schedule claims|root.*ingress"):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_root_ingress_zero_start_fusions", original_root_fusions)
            object.__setattr__(context, "v_cycle_seeded_root_ingress_route", "internal-forged")
            object.__setattr__(context, "v_cycle_seeded_core_kernel_launches", original_core_launches)
            with self.assertRaisesRegex(
                ValueError,
                "outer scalar schedule claims|root.*ingress|outer kernel schedule|seeded V-cycle core",
            ):
                dataclasses.replace(self.endpoint)
            object.__setattr__(context, "v_cycle_seeded_root_ingress_route", original_root_route)
            object.__setattr__(context, "v_cycle_seeded_core_kernel_launches", original_seeded_core_launches)
            for field_name, value in original_terminal.items():
                object.__setattr__(context, field_name, value)
    ...
    def test_coherent_literal_route_map_swap_omit_duplicate_and_generic_rehash_are_rejected(self):
        outer = self.endpoint.outer_work[0]
        record = outer.v_cycles[0]
        physical = record.physical_work
        original = (
            physical.nonterminal_literal_kernel_route,
            physical.nonterminal_literal_physical_nodes,
            physical.nonterminal_literal_physical_node_map,
        ...
        attacks = (
            ...
            (
                V_CYCLE_NONTERMINAL_LITERAL_CUDA_ROUTE,
                13,
                V_CYCLE_NONTERMINAL_LITERAL_DEFAULT_PHYSICAL_NODE_MAP + "|residual-bs3=2",
            ),
            (V_CYCLE_NONTERMINAL_GENERIC_CUDA_ROUTE, 0, ""),
        )
        try:
            for route, count, physical_map in attacks:
                with self.subTest(route=route, count=count, map=physical_map):
                    object.__setattr__(physical, "nonterminal_literal_kernel_route", route)
                    object.__setattr__(physical, "nonterminal_literal_physical_nodes", count)
                    object.__setattr__(physical, "nonterminal_literal_physical_node_map", physical_map)
                    _coherently_rehash_scalar_physical(physical)
                    _coherently_rehash_scalar_record(record)
                    with self.assertRaisesRegex(ValueError, "physical nonterminal_literal"):
                        dataclasses.replace(outer)
        finally:
            object.__setattr__(physical, "nonterminal_literal_kernel_route", original[0])
            object.__setattr__(physical, "nonterminal_literal_physical_nodes", original[1])
            object.__setattr__(physical, "nonterminal_literal_physical_node_map", original[2])
            object.__setattr__(physical, "content_sha256", original[3])
            object.__setattr__(record, "content_sha256", original[4])
    ...
    def test_nested_vcycle_signed_zero_correction_tamper_is_rejected(self):
        ...
        for cycle_index, record in enumerate(outer.v_cycles):
            with self.subTest(cycle=cycle_index):
                ...
                original_correction = record.correction
                try:
                    object.__setattr__(record, "correction", correction)
                    with self.assertRaisesRegex(ValueError, "retained correction bytes"):
                        dataclasses.replace(outer)
                finally:
                    object.__setattr__(record, "correction", original_correction)
    ...
    def test_shared_publications_reject_partial_alias_pointer_and_cycle_role_swap(self):
        ...
        for cycle_index, cycle_workspace in enumerate(cycle_workspaces):
            with self.subTest(full_publication_route=cycle_index):
                ...
                records = list(outer.v_cycles)
                records[cycle_index] = full_record
                with self.assertRaisesRegex(ValueError, "scheduled launch count is stale"):
                    dataclasses.replace(outer, v_cycles=tuple(records))
        ...
        with self.assertRaisesRegex(ValueError, "retained correction"):
    ...
    def test_record_path_requires_an_unconsumed_private_execution(self):
        ...
        try:
            with self.assertRaisesRegex(RuntimeError, "exact private sequencing receipt"):
    ...
    def test_timing_boundary_is_balanced_diagnostic_only(self):
        ...
        module = __import__("research.principal_stretch.captured_graph_vbd", fromlist=["*"])
        base_claims = module._outer_schedule_scalar_claims_from_context(timing)
        noncoarse = base_claims.hierarchy_level_count - 1
        core_launches = 2 + noncoarse * (2 + 2 * 2 + 2 * 2) - 1 - 2
        seeded_launches = core_launches - 1
        outer_launches = 8 + 2 * seeded_launches
        swapped_claims = module._require_outer_schedule_scalar_claims(
            base_claims._replace(
                pre_smooth_steps=2,
                post_smooth_steps=2,
                schedule_sha256="a" * 64,
                core_schedule_sha256="b" * 64,
                seeded_core_schedule_sha256="c" * 64,
                scheduled_kernel_launches=core_launches + 1,
                core_kernel_launches=core_launches,
                seeded_core_kernel_launches=seeded_launches,
                terminal_coarse_solve_kernel_version=V_CYCLE_TERMINAL_GENERIC_COARSE_SOLVE_KERNEL_VERSION,
                terminal_coarse_solve_route=V_CYCLE_TERMINAL_GENERIC_COARSE_SOLVE_ROUTE,
                terminal_fusion_route=V_CYCLE_TERMINAL_FUSION_CUDA_ROUTE,
                terminal_fusion_launch_reduction=2,
                terminal_collective_count=2,
                terminal_logical_phases=V_CYCLE_TERMINAL_B2_LOGICAL_PHASES,
                outer_kernel_launches_per_outer=outer_launches,
                correction_kernel_launches=2 + OUTER_CORRECTIONS * outer_launches,
            )
        )
        swapped_digest = module._derive_outer_schedule_sha256(
            timing.outer_kernel_version,
            timing.fused_gather_kernel_version,
            timing.root_seeded_gather_kernel_version,
            timing.scalar_direction_apply_kernel_version,
            timing.v_cycle_kernel_version,
            timing.v_cycle_schedule_version,
            swapped_claims,
            timing.v_cycle_publication_version,
            timing.v_cycle_standalone_publication_route,
            timing.v_cycle_external_shared_publication_route,
            timing.first_cycle_publication_role,
            timing.second_cycle_publication_role,
            timing.finalize_gate_route,
            timing.finalize_gate_block_dim,
            timing.finalize_gate_owner_threads,
            timing.finalize_gate_owner_roles,
            timing.finalize_gate_collective_version,
            timing.outer_schedule_version,
        )
        mutations = {
            "v_cycle_pre_smooth_steps": swapped_claims.pre_smooth_steps,
            "v_cycle_post_smooth_steps": swapped_claims.post_smooth_steps,
            "v_cycle_schedule_sha256": swapped_claims.schedule_sha256,
            "v_cycle_core_schedule_sha256": swapped_claims.core_schedule_sha256,
            "v_cycle_seeded_core_schedule_sha256": swapped_claims.seeded_core_schedule_sha256,
            "v_cycle_kernel_launches": swapped_claims.scheduled_kernel_launches,
            "v_cycle_core_kernel_launches": swapped_claims.core_kernel_launches,
            "v_cycle_seeded_core_kernel_launches": swapped_claims.seeded_core_kernel_launches,
            "v_cycle_terminal_coarse_solve_kernel_version": swapped_claims.terminal_coarse_solve_kernel_version,
            "v_cycle_terminal_coarse_solve_route": swapped_claims.terminal_coarse_solve_route,
            "v_cycle_terminal_fusion_route": swapped_claims.terminal_fusion_route,
            "v_cycle_terminal_fusion_launch_reduction": swapped_claims.terminal_fusion_launch_reduction,
            "v_cycle_terminal_collective_count": swapped_claims.terminal_collective_count,
            "v_cycle_terminal_logical_phases": swapped_claims.terminal_logical_phases,
            "outer_kernel_launches_per_outer": swapped_claims.outer_kernel_launches_per_outer,
            "correction_kernel_launches": swapped_claims.correction_kernel_launches,
            "outer_schedule_sha256": swapped_digest,
        }
        originals = {name: getattr(timing, name) for name in mutations}
        try:
            for name, value in mutations.items():
                object.__setattr__(timing, name, value)
            with self.assertRaisesRegex(ValueError, "live solver-issued graph claims"):
                timing.deterministic_record()
        finally:
            for name, value in originals.items():
                object.__setattr__(timing, name, value)

        with self.assertRaisesRegex(ValueError, "solver-issued timing provenance cannot be removed"):
            dataclasses.replace(
                timing,
                _validation_token=None,
                _validation_issuer=None,
                **mutations,
            )

        original_token = timing._validation_token
        original_issuer = timing._validation_issuer
        try:
            object.__setattr__(timing, "_validation_token", None)
            object.__setattr__(timing, "_validation_issuer", None)
            with self.assertRaisesRegex(ValueError, "solver-issued timing provenance cannot be removed"):
                timing.deterministic_record()
        finally:
            object.__setattr__(timing, "_validation_token", original_token)
            object.__setattr__(timing, "_validation_issuer", original_issuer)
...
@unittest.skipUnless(os.environ.get("MG_VBD_TEST_CUDA") == "1", "set MG_VBD_TEST_CUDA=1 after claiming a GPU")
class TestCapturedDirectGraphVBDRefinementCuda(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        if wp.get_cuda_device_count() < 1:
            raise unittest.SkipTest("no claimed CUDA device is visible")

    def test_refinement_construction_capture_replay_poison_and_native_nodes_scale(self):
        expected = {
            "coarse": ((14, 13, 12, 32, 130), 157, "dea8fbb40c174204ae5ad7a954bccd64a96caba64fbff1604206746198997122"),
            "medium": ((20, 19, 18, 44, 178), 205, "76e77616bd58b77f9a0eb4d1e5b8e40d3b4047f07a4cc02734a4ead012e6fab3"),
            "fine": ((26, 25, 24, 56, 226), 253, "72050b9a9918e69e7d9f816bc8071e2dee03a9c603820b916db6294eb89259d3"),
        }

        def assert_endpoint_parity(left: CapturedGraphVBDEndpoint, right: CapturedGraphVBDEndpoint) -> None:
            np.testing.assert_array_equal(left.positions, right.positions)
            np.testing.assert_array_equal(left.velocities, right.velocities)
            self.assertEqual(left.accepted, right.accepted)
            self.assertEqual(left.reasons, right.reasons)
            self.assertEqual(left.correction_kernel_launches, right.correction_kernel_launches)
            for left_outer, right_outer in zip(left.outer_work, right.outer_work, strict=True):
                for field_name in (
                    "rhs",
                    "first_correction",
                    "operator_product_after_first",
                    ...
                    np.testing.assert_array_equal(getattr(left_outer, field_name), getattr(right_outer, field_name))

        for level, (counts, native_nodes, outer_digest) in expected.items():
            with self.subTest(level=level):
                solver = CapturedDirectGraphVBD(build_refinement_scene(level), device="cuda:0")
                binding = __import__(
                    "research.principal_stretch.captured_graph_vbd", fromlist=["*"]
                )._lookup_workspace_owners(solver)
                claims = binding.claims.outer_schedule_scalar_claims
                self.assertEqual(
                    (
                        claims.scheduled_kernel_launches,
                        claims.core_kernel_launches,
                        claims.seeded_core_kernel_launches,
                        claims.outer_kernel_launches_per_outer,
                        claims.correction_kernel_launches,
                    ),
                    counts,
                )
                self.assertEqual(binding.claims.outer_schedule_sha256, outer_digest)
                uncaptured = solver.run(graph_replay=False)
                solver.capture_graphs(warmup_replays=1)
                self.assertEqual(_native_cuda_graph_node_count(solver.graph), native_nodes)
                captured = solver.run(graph_replay=True)
                assert_endpoint_parity(uncaptured, captured)
                solver.poison(seed=4817 + claims.hierarchy_level_count)
                poisoned_replay = solver.run(graph_replay=True)
                assert_endpoint_parity(captured, poisoned_replay)
                schedule = solver.deterministic_record()
                self.assertEqual(schedule["outer_schedule_sha256"], outer_digest)
                self.assertEqual(schedule["correction_kernel_launches_excluding_public_k1"], counts[-1])
                self.assertEqual(schedule["outer_kernel_launches_per_outer"], counts[-2])
...
class TestCapturedDirectGraphVBDDefaultStretchCuda(unittest.TestCase):
    ...
    def test_real_default_stretch_scalar_fused_schedule_and_physical_work_are_exact(self):
        ...
        for outer in self.endpoint.outer_work:
            ...
            for record in outer.v_cycles:
                ...
                self.assertEqual(
                    record.physical_work.terminal_coarse_solve_kernel_version,
                    V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_KERNEL_VERSION,
                )
                self.assertEqual(
                    record.physical_work.terminal_coarse_solve_route,
                    V_CYCLE_TERMINAL_FIXED12_COARSE_SOLVE_ROUTE,
                )
                self.assertEqual(record.physical_work.terminal_coarse_scalar_size, 12)
                self.assertEqual(
    ...
    def test_real_default_stretch_paired_timing_is_diagnostic(self):
        ...
        self.assertFalse(timing.performance_evidence)
        self.assertEqual(record["fused_gather_kernel_version"], FUSED_GATHER_KERNEL_VERSION)
        self.assertEqual(record["root_seeded_gather_kernel_version"], ROOT_SEEDED_GATHER_KERNEL_VERSION)
        self.assertEqual(record["scalar_direction_apply_kernel_version"], SCALAR_DIRECTION_APPLY_KERNEL_VERSION)
        self.assertEqual(record["first_cycle_publication_role"], FIRST_CYCLE_PUBLICATION_ROLE)
        self.assertEqual(record["second_cycle_publication_role"], SECOND_CYCLE_PUBLICATION_ROLE)
        self.assertEqual(record["v_cycle_publication_version"], V_CYCLE_PUBLICATION_VERSION)
